...
- S5 已批准 `Profile.StableSubList=InternalFaceN` without `ReferenceShadow` 的 request-local ElementMap-backed stable selector candidate；S6 只在该窄边界内审计或补齐 publication evidence。
...
| `backend_gap_candidate` | 只有 S3-S5 证明 oracle / current mismatch / 产品边界后，S6 才能转成实现任务或 approved candidate 发布审计。 |
...
C9-M5 queue empty
  ...
  -> S6 approved selector publication audit or no-code retained diagnostic
```
