...
- C10-M3 S0 live 基线已冻结：起点 HEAD=`f528b8f7f6`（`docs: 新增 C10-M3 ReferenceShadow native recovery 方案`），S0 起始工作区干净；主题是 `ReferenceShadow` / `ShadowSub` native recovery 准入，不是 CopyOnChange，也不是已支持声明。`C10M3-BLOCKER-000=closed_s0`，`C10M3-SCOPE-001=baseline_frozen_s0`；`C10M3-NG-001..007` forbidden claims / reopen condition 完整，`C10M3-VAL-000..005` 为 S0 docs / matrix 验收命令。C10-M3 S1 source authority audit 已完成：`C10M3-SRC-101..204` 已刷新 live FreeCAD / current cad-core path、symbol、evidence、focused tests 与 capability landing，`C10M3-BLOCKER-101=closed_s1`；没有 supported 或 backend gap 升级。C10-M3 S2 范围准入已完成：`C10M3-BLOCKER-201=closed_s2`，scope、blocker、non-goal 和 backend-gap 矩阵只发布 native oracle、current comparison、diagnostic 和 release gate 路由，没有 implementation row。S3-S6 待执行；C10-M3 的 reopen condition 是 native observable `ShadowSub` / `ReferenceShadow` evidence 加 current cad-core mismatch，否则 S6 应发布 no-code retained diagnostic。
