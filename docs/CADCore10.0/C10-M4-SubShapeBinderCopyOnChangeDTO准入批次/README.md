...
- Probe history：S1 复核了 `collect_c8m1_shapebinder_expected.py`、`C8-M2 CopyOnChange native probe（已移除）` 和 `C9-M5 CopyOnChange native probe（已移除）`；这些工具覆盖 Disabled / Enabled / Mutated / PartialLoad property states，同时仍把 full temporary-document copied-object cache 记录为 `oracle_blocked` 或 `native_oracle_blocked`。
...
- 已复核 `C9-M5 CopyOnChange native probe（脚本已移除）`、`C8-M2 CopyOnChange native probe（脚本已移除）`、`cad-core/tools/collect_c8m1_shapebinder_expected.py`、`cad-core/fixtures/c8m1` 和 `cad-core/fixtures/c8m2`；未修改 probe 或 expected。
