...
- 临时诊断字段清理已落地：`result_wire_producer_blocker_*_count`、`result_wire_producer_unknown_invariant_count`、`unowned_removal_*`、`multi_member_root_direct_output_count` 以及第四批 owner/search/exhaust JSON summary 已从 `WireJoinerLedgerSummary`、`ledgerSummary()` 和 `sketch_object.cpp` 输出删除；测试改用 producer ledger / runtime history / topo history 三方 identity 对齐和 `assertNotIn` 防回流。`helperOpenExportOverride*` 内部只删除 write-only 或可局部重算的派生字段，source lineage、root/current-member 对齐、actual output wire、`openExportOverride` 与 `resultSlotVertexEvidenceEdge` 保留。
