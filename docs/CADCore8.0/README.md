...
- C8-M6 已创建为 ShapeBinder/SubShapeBinder 下游同步源头合同主线。它不修改 `opencascade-rs`、前端或其它下游仓库，只在 FreeCAD 仓库内把 C8-M1 executor / expected、C8-M2 downstream contract 和 C8-M5 expected / diagnostics 修复后的最终口径整理成同步合同包。S0 起点为 `HEAD=b580d66ac3`（`b580d66ac3 docs: 完成 C8-M5 S6 发布闸门`），当前 S0-S6 均为待执行，工作步骤总入口已作为索引标 `【已实现】`；矩阵已 seed 出 TypeIds、capability、diagnostics、fixture expected、request-local output、known_gap、non-goal 和 release gate 验收行。`copy_on_change_full_temporary_document_cache` 继续保持 C8-M2 `known_gap` / `oracle_blocked`，本包不把它升级为 supported。
