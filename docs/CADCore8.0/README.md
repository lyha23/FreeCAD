...
- C8-M7 已创建为下一轮方案包，目标是收口 current capability 中 `topo_history.producer_matrix.import_shape.remaining=["changed_file_deleted_reference_recovery"]`。S0 已在 `HEAD=1450487296`（`1450487296 docs: 新增 C8-M7 ImportShape 恢复准入方案`）冻结 live 基线：开始工作区干净，C8-M1 到 C8-M6 队列均为空，C8-M7 队列首项为 S0；current capability 仍发布 `import_shape.status=done_first_slice`、`covered=["step","brep","iges","owner_qualified_alias"]`、`remaining=["changed_file_deleted_reference_recovery"]`。
...
- C8-M7 队列：工作步骤总入口已标记 `【已实现】`，S0 完成后只推进到 S1；S1 到 S6 不因 S0 被标记为已实现。C8-M7 不重开 C7-M7 的 imported ElementMap、ShowElement persistent writeback 或 cross-document hash / postfix lifecycle。
