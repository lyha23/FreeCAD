...
- C8-M4 已创建为下一轮方案包，目标是把 live GeomPlate gap `part_workbench.geomplate.narrowed_gaps.curve_constraint_criteria_setters_not_implemented` 按 CurveConstraint `G0Criterion` / `G1Criterion` / `G2Criterion` 同一 DTO / OCCT setter 链路批量裁决。C8-M4 起点为 `HEAD=48900289ec`（`48900289ec chore: 完成 C8-M3 S6 capability 发布闸门`）；方案明确区分 FreeCAD native `CurveConstraintPy` setter blocked 与 `cad-core` request-local product contract，不重开 GUI、`PlateSurface.Curves` wrapper lifecycle 或跨请求 geometry cache。当前只创建方案包，S0-S6 均待执行。
