...
C8-M7 已将 `topo_history.producer_matrix.import_shape.remaining=["changed_file_deleted_reference_recovery"]` 收口为 capability publication drift。最终 `import_shape` 保持 STEP / BREP / IGES / owner-qualified alias first slice，`remaining=[]`；deleted-file old geometry recovery 不进入 supported / covered。
...
- C8-M7 已完成 ImportShape changed-file / deleted-reference recovery 准入收口主线。S0 在 `HEAD=1450487296` 冻结旧 residual；S5 裁决 `changed_file_deleted_reference_recovery` 是 stale mixed publication，不是 request-local backend gap；S6 仅修改 capability publication 和 adapter capability test。
- C8-M7 最终 capability：`topo_history.producer_matrix.import_shape.status=done_first_slice`，`covered=["step","brep","iges","owner_qualified_alias"]`，`remaining=[]`，`changed_file_deleted_reference_recovery` 不再出现在 capability JSON。readable changed-file / same-file 由当前请求重导入覆盖；current imported shape + request-local `ReferenceShadow` 恢复边界已覆盖；deleted / unreadable `FileName` 只能走 diagnostic / non-goal，不声明 old geometry recovery supported。
- C8-M7 队列已收口为空。C8-M7 不重开 C7-M7 的 imported ElementMap、ShowElement persistent writeback 或 cross-document hash / postfix lifecycle，也不引入跨请求 cache、完整 BREP、TopoDS_Shape、NamedShape 或 ElementMap 持久状态。
