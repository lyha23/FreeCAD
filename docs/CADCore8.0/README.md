...
- C8-M4 已创建为下一轮方案包，目标是把 live GeomPlate gap `part_workbench.geomplate.narrowed_gaps.curve_constraint_criteria_setters_not_implemented` 按 CurveConstraint `G0Criterion` / `G1Criterion` / `G2Criterion` 同一 DTO / OCCT setter 链路批量裁决。S0 live 基线已冻结：`pwd=/home/user/Chili3DProject/FreeCAD`，`HEAD=7a9fa7bdd3`（`7a9fa7bdd3 docs: 创建 C8-M4 GeomPlate criteria 收口方案`），开始工作区干净；方案创建旧起点 `48900289ec` 只保留为历史证据。S0 已确认 C8-M3 队列为空、`part_workbench.conic_curves.remaining_gaps=[]`，C8-M4 S0 执行前队列首项为 S0，完成后下一 pending 为 S1。S0 不采 FreeCAD oracle，不新增 fixture / expected / tests / collector，不改 C++ / Rust / FreeCAD `src/`，并保留输入 gap `curve_constraint_criteria_setters_not_implemented` 与 diagnostic `unsupported_curve_criteria`；方案继续区分 FreeCAD native `CurveConstraintPy` setter blocked 与 `cad-core` request-local product contract，不重开 GUI、`PlateSurface.Curves` wrapper lifecycle 或跨请求 geometry cache。
