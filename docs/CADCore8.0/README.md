...
- C8-M4 已创建为下一轮方案包，目标是把 live GeomPlate gap `part_workbench.geomplate.narrowed_gaps.curve_constraint_criteria_setters_not_implemented` 按 CurveConstraint `G0Criterion` / `G1Criterion` / `G2Criterion` 同一 DTO / OCCT setter 链路批量裁决。S0 live 基线已冻结：`pwd=/home/user/Chili3DProject/FreeCAD`，`HEAD=7a9fa7bdd3`（`7a9fa7bdd3 docs: 创建 C8-M4 GeomPlate criteria 收口方案`），开始工作区干净；方案创建旧起点 `48900289ec` 只保留为历史证据。S1 已完成 FreeCAD source/current coverage 复核，确认 Curve DTO / parser / apply path 仍缺 request-local criteria 支持；S2 已完成 route 分类，G0/G1/G2 同批进入 `request_local_backend_gap_candidate`，native setter 保持 `native_oracle_blocked`。S3 已在 `HEAD=340ecaf864` 后完成 native boundary 复核：`CurveConstraintPyImp.cpp` 三个 setter 仍为 `PyExc_NotImplementedError("Not yet implemented")`，`PyInit` 与 getter 只证明 `GeomPlate_CurveConstraint` criteria read state；FreeCADCmd probe 可构造对象但 setter/getter runtime probe 终止且无 JSON，记录为 `environment_probe_blocked`。S4 已在 `HEAD=c2b9038d0e` 后完成 cad-core request-local gate 裁决：当前 Curve DTO 无 G0/G1/G2、`readCurveConstraints()` 仍以 `unsupported_curve_criteria` 阻断、3D Curve `addCurveConstraint()` 未调用 `SetG*Criterion()`，但 source evidence 已有 optional criteria 字段可复用；S4 打开 `open_S6_implementation_gate` 并关闭 `C8M4-BLOCKER-401`。S5 已在 `HEAD=3bbc588f8a` 后完成 capability / diagnostics / non-goal publication route：S6 实现后 request-local CurveConstraint criteria 发布为 `part_workbench.geomplate` product contract / supported，`curve_constraint_criteria_setters_not_implemented` 不得作为 active remaining gap，native setter blocked 只保留为 upstream/native parity blocked historical evidence，合法 G0/G1/G2 不再产生 `unsupported_curve_criteria`，invalid criteria 走显式 invalid finite-number diagnostic；GUI / TaskPanel / ViewProvider、`PlateSurface.Curves` wrapper lifecycle、cross-request geometry cache、downstream Rust / frontend persistence 保持 non-goal。`C8M4-BLOCKER-501` 已关闭，下一 pending 为 S6。
