...
- C8-M1 为新建方案包；工作步骤总入口是索引文件，已标记 `【已实现】`。S0 已完成 live 基线冻结，S1 已完成 FreeCAD source authority 与 current cad-core coverage 复核，S2 已完成 oracle 候选矩阵分类，S3 已完成 native oracle 批量采集与 expected 固化，S4 已完成 `cad-core` executor，S5 已完成 fixtures / tests / capability 发布；下一 pending 是 S6 release gate。
