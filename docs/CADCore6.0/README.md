...
- C6-M5 方案包已创建，目标是把 `part_workbench.filling` 的 Surface、Supports/Orders、显式参数和 non-boundary support/order 从 native helper blocker / source-backed known gap 推进为 CAD Core request-local product contract；S0-S6 当前待执行，不能标为已发布。
