...
- C6-M5 方案包已创建，目标是把 `part_workbench.filling` 的 Surface、Supports/Orders、显式参数和 non-boundary support/order 从 native helper blocker / source-backed known gap 推进为 CAD Core request-local product contract；S0 已确认 live capability 的 6 个 gap，S1 已复核 FreeCAD/cad-core 源码依据，S2 已把 Surface、Supports/Orders、显式参数和 non-boundary support/order 路由为 implementation-ready 合同，S3 已实现 Surface / SupportOrder，S4 已实现 ExplicitParams 与 non-boundary support/order；S5 已发布 capability/docs，`part_workbench.filling.remaining_gaps=[]`，native helper crash/timeout/notCollected 证据保留在 `narrowed_gaps` / historical evidence 中；S6 当前待执行，不能标为整条主线已发布。
