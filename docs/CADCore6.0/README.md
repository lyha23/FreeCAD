...
- C6-M5 方案包已创建，目标是把 `part_workbench.filling` 的 Surface、Supports/Orders、显式参数和 non-boundary support/order 从 native helper blocker / source-backed known gap 推进为 CAD Core request-local product contract；S0 已确认 live capability 仍有 6 个 `remaining_gaps`，S1 已复核 FreeCAD/cad-core 源码依据，S2 已把 Surface、Supports/Orders、显式参数和 non-boundary support/order 路由为 implementation-ready 合同，S3 已实现 Surface / SupportOrder，S4 已实现 ExplicitParams 与 non-boundary support/order；S5-S6 当前待执行，不能标为已发布或删除 `remaining_gaps`。
