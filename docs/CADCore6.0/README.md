...
- C6-M9 已实现并发布为 PartDesign Groove UpTo BRepFeat native failure 收口：S5 起点 `HEAD=e10ee2b966`（`e10ee2b966 文档：完成 C6-M9 S4 发布一致性收口`），开始时 `git status --short -uall` 为空；`cmake --build build` 通过；阶段回归 `Ran 184 tests in 92.318s`，`OK (skipped=31)`；C6-M9 队列为空，TSV 和 diff check 通过。`part_design.revolution_groove.status=supported_c51s1_advanced_with_historical_groove_upto_native_failure`，`remaining_gaps=[]`，`exact_blockers={}`，同一 id 保留在 `narrowed_gaps` / historical native evidence；两个 `c51m1` Groove UpTo fixtures 继续作为 failure-oriented guard。S3/S4 未改几何实现、fixtures、expected、collector 或 P7 failure fixture 语义；因未修改 maker history、ElementMap/history 主路径、collector expected 或批量 expected 文件，本包未触发 topology / broader expected 重型收口。
