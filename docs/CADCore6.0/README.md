...
- C6-M7 已完成 S0 live baseline 冻结：`HEAD=5caad308a9 发布 C6-M6 GeomPlate release gate`，C6-M1 到 C6-M6 队列均为空，当前队列从 S1 继续。目标是收口 `part_workbench.loft.remaining_gaps=["part_loft_subelement_assignment_native_hidden"]`：先复核 `Part::Loft.Sections` 的 `PropertyLinkList` 原生可见性，再判定 request-local product contract 或 native-hidden diagnostic / narrowed gap 路由。C6-M7 不重开 C5-M12 已关闭的 complex profile family，不声明 `PartDesign::Loft`、GUI、FreeCAD parity 或 full Part surface family。
