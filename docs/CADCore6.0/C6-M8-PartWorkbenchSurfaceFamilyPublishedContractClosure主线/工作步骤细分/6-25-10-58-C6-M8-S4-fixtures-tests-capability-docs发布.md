...
把 S3 结果发布到 capability、adapter assertion、C6-M8 docs 和 root README。S4 负责让用户和前端看到一致的 surface family 合同。

## 动作

1. 更新 `cad-core/src/runtime/capability_contract.cpp` 的 `part_workbench` surface family 状态。
2. 更新 `cad-core/tests/test_adapters.py::test_c_api_capabilities_exposes_web_contract_facts`。
3. 更新 C6-M8 矩阵，把 S2/S3 route 写成 published / closed / non-goal。
4. 更新 `docs/CADCore6.0/README.md` 的 C6-M8 当前状态。
5. 若 S3 新增 fixtures 或 expected/product metadata，确认 fixture list、object_fields、diagnostics 和 capability evidence 一致。
