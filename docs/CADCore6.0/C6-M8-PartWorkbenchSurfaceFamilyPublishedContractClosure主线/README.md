...
- live 起点：`pwd=/Users/li/Chili3DProject/FreeCAD`，S0 起点 `HEAD=02798d9ca9`（`02798d9ca9 docs: 新增 C6-M8 表面族合同收口方案`）；S1 起点 `HEAD=f1f32b19c8`（`f1f32b19c8 文档：完成 C6-M8 S0 live 基线冻结`）；S2 起点 `HEAD=f981730d1c`（`f981730d1c 文档：完成 C6-M8 S1 源码与矩阵复核`）；S3 起点 `HEAD=72f173f682`（`72f173f682 文档：完成 C6-M8 S2 ProjectOnSurface 路由裁决`），四次开始时 `git status --short -uall` 均为空；C6-M1 到 C6-M7 `工作步骤细分` 队列均为空。
- S0/S1/S2/S3 已实现：S0 冻结 live baseline；S1 已复核六个 owner 的 FreeCAD source authority、cad-core 落点、checked-in fixtures、product fixtures、diagnostic evidence 和 adapter assertion；S2 已裁决 ProjectOnSurface overlap；S3 已按 S2 route 收口 capability + adapter assertion + docs/matrices，当前队列推进到 S4。
...
- C6-M8 的下一优先级是 S4 publication docs：确认 ProjectOnSurface S3 发布口径与 RuledSurface / Loft / Sweep / Filling / GeomPlate 的 `remaining_gaps=[]`、`narrowed_gaps`、`non_goals` 文档一致。
