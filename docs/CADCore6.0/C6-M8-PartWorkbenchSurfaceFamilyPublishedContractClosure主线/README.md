...
- C6-M8 S5 release gate 已通过：`cmake --build build` 通过；`python3 -m unittest tests.test_p8_features tests.test_expected_fixtures tests.test_adapters` 为 `Ran 254 tests in 87.456s`，`OK (skipped=31)`；queue、TSV 字段检查和 diff check 通过。因 S3/S4 未修改 `topo_shape_expansion`、ElementMap/history 主路径、collector expected 语义或批量 expected 文件，本包未触发 topology / broader expected 重型收口。
