...
本文是 `docs/CADCore6.0` 下的 C6-M6 实施主线。当前已完成 S0 live 基线冻结、S1 source / wrapper / oracle 候选矩阵、S2 remainingGap 准入路由和 S3 G1 / ProjectedCurve2d 合同实现或收窄，S4 到 S6 仍 pending。执行时必须从 `工作步骤细分/` 的 live queue 逐步推进，不能跳过实现、fixture、capability 和 release gate 直接删除 `remaining_gaps`。
