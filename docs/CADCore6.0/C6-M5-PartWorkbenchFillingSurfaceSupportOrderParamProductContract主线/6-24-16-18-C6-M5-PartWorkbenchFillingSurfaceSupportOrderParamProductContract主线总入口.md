...
- `cad-core/src/runtime/capability_contract.cpp` 中 `part_workbench.filling` 当前状态是 `supported_expected_backed_plus_c6m5_product_contract_non_parity`。
...
- S5 已将六个 S0 缺口从 `remaining_gaps` 删除；这些 native helper crash / timeout / notCollected 证据仍作为 `narrowed_gaps` 与 historical native helper evidence 保留，不声明 FreeCAD parity。
