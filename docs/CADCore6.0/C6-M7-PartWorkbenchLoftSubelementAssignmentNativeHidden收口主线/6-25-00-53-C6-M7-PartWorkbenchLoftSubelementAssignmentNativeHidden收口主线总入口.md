...
- C6-M1 到 C6-M6 `step_goal_queue.py` 均返回空表；S1 完成后 C6-M7 队列从 S2 继续。
...
- `cad-core/src/part/part_loft.cpp` 当前只通过 `app::readLinks(object, "Sections")` 读取 object-level sections；`cad-core/src/part/topo_shape_expansion.cpp::makeElementLoftFromSources()` 已有 `BRepOffsetAPI_ThruSections` 和 thru-sections history 落点。因此 S1 不把现状写成 C++/topo 缺口，只把 selected subelement 支持保留为 S2 待判定的 request-local DTO 候选。
...
C6-M7 应先处理 Loft 唯一 active remaining gap，再考虑 Surface Family freeze。S1 已证明 FreeCAD 原生 `Sections` 边界仍是 object-level `PropertyLinkList` native-hidden evidence；S2 才能决定是否引入 CAD Core request-local non-parity DTO。若 S5 后 `part_workbench.loft.remaining_gaps=[]`，下一步才适合开 CADCore6 surface family freeze / publication audit 包。
