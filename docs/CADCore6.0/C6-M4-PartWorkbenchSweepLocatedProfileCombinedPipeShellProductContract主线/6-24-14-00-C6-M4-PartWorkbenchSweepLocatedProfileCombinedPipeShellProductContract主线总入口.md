...
| cad-core executor | `cad-core/src/part/part_sweep.cpp` | 解析 `SectionOptions[].Location/WithContact/WithCorrection`、`AuxiliarySpine`、`Tolerance` 并写 advanced metadata；Location build failure 转 `known_gap`，不发布 `named_shapes`。 |
| low-level cad-core PipeShell | `cad-core/src/part/topo_shape_expansion.cpp::makeElementPipeShellFromSources()` | 已有 `PipeShellSectionOption`、`location`、`tolerance`、`auxiliary` 槽位；当前调用 OCCT `Add(profile, vertex, ...)`，失败后由 executor 维持 known_gap。 |
...
| `矩阵/c6m4_sweep_located_profile_combined_scope_review_matrix.tsv` | scope、状态、落点、下一步。 |
