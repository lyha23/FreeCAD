...
- S0-S6 已实现；S6 阶段回归与 release gate 已通过，当前队列为空。
...
- S6 已通过发布闸门：`cmake --build build` 通过；阶段回归 `Ran 241 tests in 65.269s`，`OK (skipped=29)`；heavy 收口 `Ran 277 tests in 71.034s`，`OK (skipped=29)`；`C6M4-BLK-501` 已关闭，TSV 字段一致性、`git diff --check` 和队列检查通过。
