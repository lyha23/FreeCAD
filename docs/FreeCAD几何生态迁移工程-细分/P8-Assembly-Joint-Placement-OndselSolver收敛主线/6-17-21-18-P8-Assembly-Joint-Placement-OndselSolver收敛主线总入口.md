...
- `cad-core/tests/test_p8_features.py` 已覆盖 P8 Assembly input metadata、C3M6 grounded real solver、无 GroundedJoint solved/no updates native 行为、multi-component writeback、invalid grounded writeback 和 unsupported joint diagnostics；`cad-core/fixtures/c3m6/expected` 已提供并通过 FreeCADCmd native placement oracle。
- `cad-core/src/adapters/c_api/c_api.cpp` 的 `assembly` capabilities 只发布 `ondsel_solver_adapter`、`placement_writeback` 和 `unsupported_joint_matrix` 子集；P8 正式文档已同步 real-only 口径。
