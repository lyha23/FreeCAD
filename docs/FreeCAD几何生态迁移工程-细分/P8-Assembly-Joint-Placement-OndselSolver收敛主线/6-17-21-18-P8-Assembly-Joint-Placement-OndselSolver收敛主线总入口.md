...
- 把正式 P8 文档里仍保留的 `solve=not_migrated` 口径，与当前 C++、C ABI capabilities、fixtures 和 focused tests 对齐。
...
- `cad-core/src/adapters/c_api/c_api.cpp` 的 `assembly` capabilities 已发布 `representative_solver_adapter`、`ondsel_solver_adapter` 和 `placement_writeback` 子集；P8 正式文档仍需要 S0/S6 复核并回写。
...
| S3 Ondsel adapter | `工作步骤细分/6-17-21-22-【已实现】P8-AssemblySolver-S3-OndselSolver适配专项复审.md` | 已完成：复核 real solver / representative fallback / build link 边界，当前 unlinked build 缺口路由 S6 |
| S4 writeback | `工作步骤细分/6-17-21-23-【已实现】P8-AssemblySolver-S4-PlacementWriteback生命周期专项复审.md` | 已完成：复核 `documentObjectUpdates`、grounded validation、下一请求应用、多组件顺序；focused 失败归因 S3 build-mode 并路由 S6 |
...
| S6 发布闸门 | `工作步骤细分/6-17-21-25-P8-AssemblySolver-S6-Oracle实现与发布闸门.md` | 消费 blocker，落 C++ / expected / docs 发布 |
...
当前工作步骤总入口只是索引，已完成创建；S0 已完成 live 基线复核并确认 publication drift：正式 P8 文档仍保留 `solve=not_migrated` / 不执行 OndselSolver 的旧口径，而当前 C++、C ABI capabilities、focused tests 和 fixtures 已暴露 request-local solver adapter、representative fallback、可选 real Ondsel adapter、placement writeback 和 unsupported joint diagnostics 子集。S1 已完成源码候选矩阵补证，只建立 source authority；S2 已完成范围准入与 blocker 矩阵裁决，把当前主线拆为 releaseGate、notCollected、unsupported 和 nonGoal，没有凭旧文档创建 backendGap。S3 已完成 Ondsel adapter 专项复审：real adapter 只在 `CAD_CORE_HAS_ONDSEL_SOLVER=1` 时可声明，当前本地 build 为 `=0` 并触发 representative fallback，native solver placement oracle 仍为 `notCollected`。S4 已完成 PlacementWriteback 生命周期专项复审：writeback 只作为 `documentObjectUpdates` 的 `Placement` 更新建议，下一请求由前端 graph 消费，focused test 失败归因 S3 build-mode/test-route 而非 writeback contract 自身；S5 已完成 JointType 裁决：当前 releaseGate 子集只有 Fixed / Revolute / Slider / Ball / Distance / Angle，Cylindrical / Parallel / Perpendicular / RackPinion / Screw / Gears / Belt 保持 diagnostic-only，复杂 Distance geometry 仍 `notCollected`。S6 仍待执行，不得把完整 Assembly solver 写成已实现。
