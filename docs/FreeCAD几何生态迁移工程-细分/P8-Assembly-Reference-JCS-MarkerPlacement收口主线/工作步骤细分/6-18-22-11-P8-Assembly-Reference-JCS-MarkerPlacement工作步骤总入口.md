...
当前收口状态：本文件是执行索引。S0 已完成 live 基线复核；S1 已完成 FreeCAD 源码候选矩阵复核；S2 已完成范围准入与 blocker 矩阵复核；S3 已完成 MarkerPlacementResolver evidence / diagnostic 前置；S4 已完成 NativeOracle 与代表 fixture 专项复审；S5-S6 尚未执行，不得把本方案写成已支持。
...
5. S4 已批量采集 object / vertex / edge / face / mixed / current value / special rewrite representative expected；subshape expected 带 `known_gap` 等待 S5 parity 删除。
...
| `p8_marker_placement_scope_review_matrix.tsv` | 当前能力路由 | S4 后 14 个 scope：`supportedBaseline=1`、`backendGap=4`、`oracleCollected=5`、`releaseGate=3`、`nonGoal=1`，其中 current-value oracle 已采集但 mixed parity 仍属 backendGap |
| `p8_marker_placement_blocker_queue.tsv` | 发布前 blocker | S4 已关闭 `MP-BLOCK-004` 并完成 `MP-BLOCK-005` 的 oracle 部分；S5-S6 仍需 real Ondsel parity、special regression、capability 和 boundary audit |
