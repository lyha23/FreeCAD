...
当前收口状态：S0 已完成 live baseline 复核，S1 已完成 FreeCAD 源码候选矩阵复核，S2 已完成范围准入与 blocker 路由，S3 已完成 `Distance2` DTO 与 `ASMTGearJoint` runtime 映射；S4 到 S6 仍为待执行。尚未完成 native oracle、focused tests 或 capability 发布闸门；不得把整个主线写成“已实现”。
...
| 矩阵 | 当前用途 | 当前结论 |
| --- | --- | --- |
| `p8_gears_belt_joint_source_candidates.tsv` | FreeCAD / cad-core 证据入口 | 已记录 S3 DTO / ASMT 映射代码关闭，当前 unsupported publication 和 test 落点仍待 S4/S5 |
| `p8_gears_belt_joint_scope_review_matrix.tsv` | 当前能力路由 | GBJ-SCOPE-002 / 003 代码已落地但仍等 native expected / capability；GBJ-SCOPE-004 仍 notCollected |
| `p8_gears_belt_joint_blocker_queue.tsv` | 发布前 blocker | GBJ-BLOCK-001 / 002 已由 S3 关闭；oracle、capability、remaining unsupported boundary 待关闭 |
