...
C9-M5 承接 C9-M4 queue-empty 后的 live capability gap：`part_design.sub_shape_binder.copy_on_change_full_temporary_document_cache`。它不直接实现 FreeCAD temporary document cache，而是复审 `SubShapeBinder::setupCopyOnChange()` / `update()`、`LinkBaseExtension::syncCopyOnChange()` 和 `Document::copyObject()` 是否能导出稳定、产品批准、完全 request-local 的 CopyOnChange DTO。S4 已裁定 `dto_rejected_known_gap_retained`，S5/S6 已关闭为 no-code `known_gap_diagnostic` / `oracle_blocked` release gate，不能落 C++ support；`C9M5-BLOCKER-601` 已关闭，C9-M5 队列为空。
...
- C9-M5 方案包已关闭：生成基线为 `HEAD=ceef6a128b`（`ceef6a128b feat: 关闭 C9-M4 S6 默认距离类型发布闸门`）且生成前工作区干净；S0 执行基线仍为 `HEAD=ceef6a128b`，S1 执行基线为 `HEAD=4f2416c8dd`（`4f2416c8dd docs: 关闭 C9-M5 S0 基线冻结`），S2 执行基线为 `HEAD=4a7e8028d1`（`4a7e8028d1 docs: 关闭 C9-M5 S1 源码覆盖审计`），S3 执行基线为 `HEAD=11eab67134`（`11eab67134 docs: 关闭 C9-M5 S2 范围准入矩阵`），S4 执行基线为 `HEAD=14c20f0b12`（`14c20f0b12 docs: 关闭 C9-M5 S3 native CopyOnChange 复审`），S5 执行基线为 `HEAD=b9f370ab76`（`b9f370ab76 docs: 关闭 C9-M5 S4 DTO 产品边界复审`），S6 执行基线为 `HEAD=c68a9621b5`（`c68a9621b5 docs: 关闭 C9-M5 S5 diagnostic 发布复审`），各步起始 status 均 clean。C9-M4 队列为空，当前 live gap 仍是 `part_design.sub_shape_binder.copy_on_change_full_temporary_document_cache`，发布状态为 `known_gap_diagnostic` / `oracle_blocked`；S1 已复核 FreeCAD `_tmp_binder` / `_CopiedObjs` / `copyObject()` / `recomputeFeature(true)` / `_CopiedLink` full path 和 current cad-core diagnostic/capability/tests 边界；S2 已把 full temporary-document cache 保持为 `known_gap_retained`，DTO 只保留 `backend_gap_candidate` / `product_decision_required`；S3 新增 C9-M5 专属 native probe / fixture / expected，FreeCADCmd `1.2.0 revision 20260519` 证据确认 property-state、`_tmp_binder` 和 `_CopiedLink` 可见，但 `_CopiedObjs`、`copyObject()` dependency order 和 `recomputeFeature(true)` lifecycle 仍不可序列化为稳定 request-local DTO；S4 裁决为 `dto_rejected_known_gap_retained`，允许字段只限 request graph、request-local documentObjectUpdates / diagnostics、source/support/mutation intent 和 deletion/update/reselect 建议，forbidden fields 仍禁止 hidden temporary document、BREP / TopoDS_Shape / full shape cache、request 后继续有效的 NamedShape / ElementMap / copied-object cache、`_CopiedObjs` 和 native pointer identity；S5/S6 已确认 current cad-core 仍发布 `copy_on_change_full_temporary_document_cache_not_supported` 和 `remaining_gaps=["copy_on_change_full_temporary_document_cache"]`，最终裁决为 `no_code_retained_known_gap_release_gate`，`C9M5-BLOCKER-601` 已关闭，C9-M5 队列为空。
