...
- C9-M5 方案包已建立，S0 和 S1 已关闭且 S2-S6 待执行：生成基线为 `HEAD=ceef6a128b`（`ceef6a128b feat: 关闭 C9-M4 S6 默认距离类型发布闸门`）且生成前工作区干净；S0 执行基线仍为 `HEAD=ceef6a128b`，S1 执行基线为 `HEAD=4f2416c8dd`（`4f2416c8dd docs: 关闭 C9-M5 S0 基线冻结`）且起始 status clean。C9-M4 队列为空，当前 live gap 仍是 `part_design.sub_shape_binder.copy_on_change_full_temporary_document_cache`，发布状态为 `known_gap_diagnostic` / `oracle_blocked`；S1 已复核 FreeCAD `_tmp_binder` / `_CopiedObjs` / `copyObject()` / `recomputeFeature(true)` / `_CopiedLink` full path 和 current cad-core diagnostic/capability/tests 边界，未把该 gap 写成 supported 或 `backend_gap_requires_implementation`。
