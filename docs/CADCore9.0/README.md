...
- C9-M3 S0-S1 已关闭：C9-M2 handoff commit 为 `b981e84f68`，S0 live 执行起点为 `04bdd2e561 docs: 新增 C9-M3 DistanceType default boundary 方案`，S1 live 执行起点为 `8f209aab54 docs: 关闭 C9-M3 S0 基线冻结`；C9-M2 queue 为空，Assembly `remaining_gaps=[]`、`unsupported_joint_matrix=[]`，但 `PointCurve` deferred diagnostic 与 `default_or_todo_boundaries` 仍公开。S1 已固化 FreeCAD `getDistanceType()` / `makeMbdJointDistance()` source authority、current cad-core landing、collector metadata、checked-in expected inventory 和 diagnostics guard；S2-S6 仍待执行。`PointCurve`、`PlaneCone`、`LineCylinder`、`CurvePlane`、`Other` 已有 checked-in expected 但仍保留 diagnostic/default boundary，其他 default planar branch rows 需要 S4 批量采集或明确 `notCollected`。S6 只消费 expected-backed backendGap 或 releaseGate，不把未采集 row 写成 supported。
