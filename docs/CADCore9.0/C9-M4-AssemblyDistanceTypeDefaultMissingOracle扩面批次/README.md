...
| 方向 | 当前状态 | C9-M4 目标 |
| --- | --- | --- |
| Face / Face cone family | `CylinderCone`、`ConeCone`、`ConeTorus`、`ConeSphere` 缺 input / expected | S3 采集 native expected 或记录 native oracle blocker。 |
...
| S6 implementation gate | 仅 accepted native expected rows 可进入实现 | 实现 expected-backed mismatch；missing rows 保持 visible default boundary。 |
