...
- C9-M3 `工作步骤细分` 队列已复核为空；C9-M4 S0 已冻结起点，S1 已关闭 source authority/current coverage 候选，S2 已关闭 scope / blocker / non-goal 初始路由，S3 已关闭 FaceCone native oracle，S4 已关闭 PointLineSurface native oracle，S5 已关闭 CurveSurface native oracle，S6 仍待执行。
...
- S3 已为 `CylinderCone`、`ConeCone`、`ConeTorus`、`ConeSphere` 新增 c3m6 input / expected，native FreeCAD expected 均为 solved + placement writeback，current cad-core 保持 `unsupported/default_boundary_not_mapped`，因此四行路由为 S6 `backend_gap_candidate`。S4 已为 `PointCone`、`PointTorus`、`LineSphere`、`LineCone`、`LineTorus` 新增 c3m6 input / expected，FreeCADCmd `1.2.0 revision 20260519` native expected 同样为 solved + placement writeback，current cad-core 保持 `unsupported/default_boundary_not_mapped`，因此五行路由为 S6 `backend_gap_candidate`。S5 已为 `CurveCylinder`、`CurveSphere`、`CurveCone`、`CurveTorus` 新增 c3m6 input / expected，native expected 为 solved + Face-first `swapJCS` ordering + placement writeback，current cad-core 保持 `unsupported/default_boundary_not_mapped`，因此四行也路由为 S6 `backend_gap_candidate`。
...
| Point / Line + Surface family | `PointCone`、`PointTorus`、`LineSphere`、`LineCone`、`LineTorus` expected-backed current unsupported | S4 已关闭，五行交 S6 `backend_gap_candidate`。 |
| Curve + Surface family | `CurveCylinder`、`CurveSphere`、`CurveCone`、`CurveTorus` expected-backed current unsupported | S5 已关闭，四行交 S6 `backend_gap_candidate`；conic publication mirror 仍保留四行 default boundary。 |
