...
- 收口 `future_sweep_advanced_contract`：已实现字段发布 expected-backed 或 diagnostic-backed；仍无法 oracle 的字段必须有 source-backed known_gap、delete_condition 和下一 owner。
...
| capability closeout | docs、capability metadata、root matrices、remaining gaps | S4 关闭 broad `future_sweep_advanced_contract`，只保留精确 future owner 或 known_gap |

## S0 状态

S0 已冻结 live baseline `cd4a092d9a`：`part-sweep-multi-profile-linearize` 继续作为 C5-M6/C4M1 expected-backed multi-profile / `Linearize=true` guard；`part-sweep-advanced-deferred` 只保护 `AuxiliarySpine`、`Tolerance` 等 advanced 字段的 locatable `unsupported_property` diagnostics。当前 capability 仍只发布基础 Sweep slice，`future_sweep_advanced_contract` 不在 S0 关闭。
