...
本包承接 C5-M10 已关闭的 `part_workbench.sweep` advanced PipeShell 字段级合同。C5-M11 已删除 broad `part_sweep_wrapper_expected_collector`：`AuxiliarySpine`、`AuxiliaryCurvilinear`、`Binormal` / `BiNormal`、`Tolerance.tol3d` / `boundTol` / `tolAngular` 已有 FreeCADCmd wrapper expected；`SpineSupport` / `SupportMode`、`SectionOptions[].Location` / `WithContact` / `WithCorrection` 和 `advanced_combination` 只保留明确窄化 blocker。

C5-M11 的目标不是新增字段，而是围绕同一 `Part.BRepOffsetAPI_MakePipeShell` wrapper API 做 expected-backed 批量闭环：新增 FreeCADCmd request-local wrapper collector，替换 collectable expected，并把 capability、focused tests、fixture expected 和文档矩阵同步成 expected-backed / narrowed blocker 状态。
...
- 方案：`6-22-00-23-【已实现】C5-M11-PartWorkbenchSweepWrapperExpectedParity方案.md`
...
| tests / capability | expected fixture assertions、adapter capability、remaining gap 清理 | S3 focused tests 和 capability metadata 把 collectable 字段从 source-backed known_gap 晋级 expected-backed，并删除 broad collector gap |
...
- support diagnostics fixture 当前没有 valid support representative，保留 diagnostic-only narrowed blocker；后续只有新增或复用 valid support representative 后，才能晋级 wrapper expected-backed。
