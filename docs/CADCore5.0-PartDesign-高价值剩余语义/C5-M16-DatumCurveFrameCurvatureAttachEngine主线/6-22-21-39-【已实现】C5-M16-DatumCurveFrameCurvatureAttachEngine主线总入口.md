# 【已实现】C5-M16 DatumCurveFrameCurvature AttachEngine 主线
...
- C5-M15 S6 已关闭；C5-M16 S6 在此基础上完成本包 expected-backed release gate。
- exact blocker 只移除本包 proven modes；excluded family 仍留在 capability blocker 或 non-goal。
...
- M15 已关闭 3D plane family，不负责 curve frame / curvature。
- C5-M16 S6 已从 capability exact blocker 移除 expected-backed proven modes：`FrenetNB`、`FrenetTN`、`FrenetTB`、`Concentric`、`SectionOfRevolution`、`AxisOfCurvature`、`Normal`、`Binormal`、`CenterOfCurvature`。
- capability exact blocker 仍保留 `Folding`、conic landmarks、`IntersectionPoint`、`TangentU/V` 等 excluded family。
...
| 类型 | 路径 | 用途 |
| --- | --- | --- |
| 方案 | `6-22-21-39-【已实现】C5-M16-DatumCurveFrameCurvatureAttachEngine方案.md` | 主线范围、实现顺序、验收分层 |
...
| S4 | `工作步骤细分/6-22-21-45-【已实现】C5-M16-S4-CurvatureCenterAlias专项复审.md` | curvature center 与 line/point alias |
| S5 | `工作步骤细分/6-22-21-46-【已实现】C5-M16-S5-requestLocalPlacementAndCapability专项复审.md` | writeback、capability、release boundary |
| S6 | `工作步骤细分/6-22-21-47-【已实现】C5-M16-S6-Oracle实现与发布闸门.md` | oracle、C++、fixtures、tests、docs closeout |
...
- S4 已完成 CurvatureCenter / Alias 合同冻结：`SectionOfRevolution` 先用 `T.Reversed` / `N.Reversed`，`Concentric` 先用 `B` / `T`，再用 `curvature = dd.Dot(N) / pow(d.Magnitude(), 2)` 把 base 移到 `p + N * (1 / curvature)`；`N == 0` 必须报 infinite radius / undefined curvature diagnostic。`AxisOfCurvature`、`Normal`、`Binormal`、`CenterOfCurvature` 只复用 shared 3D curve-frame / curvature helper，`AxisOfCurvature` 额外应用 Z-to-Y presuper rotation，line/point executors 不复制曲线帧业务规则。
- S5 已完成 request-local placement / capability 合同冻结：curve-frame helpers 只消费本次 request graph 的 support shape、subname、`MapPathParameter`、`AttachmentOffset`、`MapReversed`；response 只返回 placement、diagnostics 和 `documentObjectUpdates` / `elementReferenceUpdates` suggestions；`ReferenceShadow.brep` 只保留为单个被引用 subshape 的旧几何快照例外，不得扩展成完整 BREP、建模输入或跨请求缓存。
- S6 已完成 Oracle 实现与发布闸门：`partdesign-datum-curve-frame-modes` 和 `partdesign-datum-curve-frame-diagnostics` expected 均为 FreeCADCmd-backed；cad-core helper、DatumLine/DatumPoint alias、focused tests、adapter capability、package/root matrices 已同步。`projection_failed` 是 source/code-path-backed diagnostic，但无稳定 primitive fixture；不影响本轮 supported modes 的 expected-backed 发布。
