...
| C5-M17 Datum Remaining AttachEngine（新开包） | `C5-M17-DatumRemainingAttachEngine主线/6-23-03-26-C5-M17-DatumRemainingAttachEngine主线总入口.md` | `C5-M17-DatumRemainingAttachEngine主线/工作步骤细分/` |
...
C5-M17 打开 Datum remaining AttachEngine 主线：它承接 C5-M16 后 `datum_attach_engine_remaining_modes` 的剩余项，先冻结 live blocker 和 FreeCAD source route，再把同一语义批次拆开推进。第一批可执行 owner 是 conic landmarks：DatumLine `Directrix1/2`、`Asymptote1/2` 和 DatumPoint `Focus1/2`；这些共享 `AttachEngineLine` / `AttachEnginePoint` conic edge route、`BRepAdaptor_Curve` 与 gp conic APIs，适合在同一 `c51m5` expected family 下做 oracle、C++、fixtures、focused tests 和 capability closeout。`Folding` 是四线 fold-angle 状态机，`IntersectionPoint` 需要 face/face DTO 与 oracle，`TangentU/V` 属 surface tangent adjacent family，均不进入 conic S6，仍保留 exact blocker 或 later-package guard。
