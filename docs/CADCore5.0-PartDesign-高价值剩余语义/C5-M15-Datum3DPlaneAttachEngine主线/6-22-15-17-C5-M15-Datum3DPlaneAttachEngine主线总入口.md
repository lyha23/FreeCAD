# C5-M15 Datum3DPlane AttachEngine 主线

状态：`s5_request_local_capability_review_verified__S6_pending`

本包承接 C5-M14 DatumPoint `ProximityPoint1/2` 已收口后的下一批 AttachEngine exact blocker。它不再挑单个 thin case，而是按同一 FreeCAD 调用链、同一 cad-core DTO/API 边界、同一 c51m5 expected 家族，批量处理 `Translate`、`TangentPlane`、`ThreePointsPlane`、`ThreePointsNormal`。

## 目标

- 把 `datum_attach_engine_remaining_modes` 中的 3D plane / local placement family 从大 blocker 拆成可执行批次。
- 冻结 FreeCAD `AttachEngine3D::_calculateAttachedPlacement()` 中四个 mode 的源码语义：`Translate` 的 vertex-local placement，`TangentPlane` 的 face+vertex surface projection/normal/tangent，`ThreePointsPlane` 和 `ThreePointsNormal` 的三点成面和法向推导。
- 在方案阶段写清批量采集 oracle、补 `cad-core/src/part_design/datum_attachment.h`、新增 `c51m5` fixtures/expected、focused tests、capability/docs 和验收记录闭环。
...
- 发布时只从 exact blocker 移除本包四个 mode；`Folding`、Frenet/curve frame、curvature/conic landmarks、`IntersectionPoint` 保持后续分包。

## 当前基线

- C5-M14 已在 cad-core capability 中移除 `ProximityPoint1/2`，并已有 `c51m5/partdesign-datum-point-proximity-*` expected-backed fixtures。
- live exact blocker 仍列出 `Translate`、`TangentPlane`、`ThreePointsPlane`、`ThreePointsNormal`，同属 `AttachEngine3D::_calculateAttachedPlacement()`。
- `cad-core/src/part_design/datum_attachment.h` 已能识别 MapMode 名称，但本批四个 3D mode 仍未进入 selected placement 主路径。
- `cad-core/fixtures/c51m5` 是 Datum AttachEngine expected 家族，本包继续使用该目录，不新建跨主题 fixture 家族。
...
| 层 | 当前代码落点 | 职责 |
| --- | --- | --- |
| PartDesign Datum AttachEngine | `cad-core/src/part_design/datum_attachment.h` | 新增四个 3D selected MapMode 的 request-local placement helper、support 解析、diagnostics |
...
| fixture / expected | `cad-core/fixtures/c51m5` | 新增 `partdesign-datum-3d-plane-modes` 和 diagnostics fixture |
...
| 类型 | 路径 | 用途 |
| --- | --- | --- |
| 方案 | `6-22-15-17-C5-M15-Datum3DPlaneAttachEngine方案.md` | 主线范围、实现顺序、验收分层 |
...
| S6 | `工作步骤细分/6-22-15-25-C5-M15-S6-Oracle实现与发布闸门.md` | oracle、代码落点、focused tests、发布闸门 |
...
| oracle batch | vertex translate、three vertices、edge endpoints + vertex、face+vertex tangent plane、invalid diagnostics | S6 FreeCADCmd expected 与 focused tests |
| implementation batch | `datum_attachment.h` selected 3D placement helpers | C++ code landing，不放 adapter 业务逻辑 |
| release closeout | fixtures、tests、capability exact blocker、C5/C51 docs | 发布前只移除本包四个 mode |
