...
- S3/S4 已产生或裁决的 fixture、expected、tests 和源码证据。

## 执行要点

1. 记录 live baseline 和 C7-M7 queue。
2. 复跑本包 queue、TSV 列数、trailing whitespace 和 `git diff --check`。
3. 若 S5 未改 C++、fixture、expected、test 或 capability，默认不跑 `cmake --build build` 和 focused unittest。
4. 若 S5 或 S6 额外改了代码 / fixture / expected / capability，再复跑 `cmake --build build` 和 focused unittest。
...
rg -n '[ \t]$' docs/CADCore7.0/C7-M7-P8LinkElement持久写回与导入ElementMap完整闭环主线 docs/CADCore7.0/README.md docs/CADCore方案/细化方案/11-P8-Part导入导出与Assembly后续.md
git diff --check
```
