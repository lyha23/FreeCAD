...
- S1 已关闭 `C7M7-BLOCKER-101` / `C7M7-GATE-101`，没有采 oracle、没有新增 fixture/expected/test、没有改 C++；S2 输入池是完整 imported-shape `ElementMap`、ShowElement LinkElement / LinkGroup persistent writeback transaction、复杂多层 LinkSub / cross-document hash-postfix lifecycle，下一队列项是 S2 oracle 候选矩阵。
