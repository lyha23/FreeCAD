...
- S5 已关闭 `C7M6-BLOCKER-501` / `C7M6-GATE-601`：implementation gate closed，没有 `backend_gap_requires_implementation`，本轮只做 README / 方案 / 总入口 / 工作步骤 / 矩阵 no-code publication closure；未改 C++、adapter、tests、fixtures、expected、collector 或生成输出。`C7M6-GATE-701` / `C7M6-BLOCKER-601` 仍等待 S6 release gate。
