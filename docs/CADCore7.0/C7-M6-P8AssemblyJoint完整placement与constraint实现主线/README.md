...
- S4 已关闭 `C7M6-BLOCKER-401` / `C7M6-GATE-601`：`C7M6-ORACLE-202` 以 current `cad-core` legacy recompute 和 focused P8 unittest 裁为 `already_closed_expected_backed`，S3 expected 里的 marker `backendGap` 只是历史 collector 元数据；`C7M6-ORACLE-302` / `C7M6-ORACLE-203` 继续 `oracle_blocked`。S5 implementation gate closed，只允许 no-code publication closure，不允许改 C++、adapter、tests、fixtures 或 expected。
