...
- S0 已关闭 `C7M6-BLOCKER-000` / `C7M6-GATE-000`；S1 已关闭 `C7M6-BLOCKER-101`；S2 已关闭 `C7M6-BLOCKER-201`。S2 只更新文档和矩阵，没有采 oracle、没有新增 fixtures/expected/tests、没有改 C++；已选定 `C7M6-ORACLE-202` / `C7M6-ORACLE-302` 两个 S3 native oracle candidates，`C7M6-ORACLE-203` 保持 offsetPlc `oracle_blocker`，没有 S2 `backend_gap_candidate`。队列推进到 S3；S4 裁决 implementation gate，S5 只有在 S4 打开 `backend_gap_requires_implementation` 时才改 C++。
