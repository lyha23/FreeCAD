...
工作步骤总入口索引已按 C7-M1/C7-M2/C7-M3 约定标记为 `【已实现】`，只用于跳过索引文件；S0 已冻结 live 基线与旧 P7T 已关闭边界。当前下一 pending 必须是 S1。
...
2. S1：下一 pending，复核 FreeCAD source、当前 `cad-core` topo/history 能力和 fixture/test 覆盖。
