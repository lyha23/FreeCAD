...
| `src/topo/reference_matcher.cpp` | `src/app/property_links.cpp`、`src/part/topo_shape_expansion.cpp` | `PropertyLinks.cpp`、`TopoShapeExpansion.cpp` | 引用解析和 shape history 恢复分层处理。 |
...
允许保留少量真正无 FreeCAD 业务语义的 OCCT 工具，例如 bbox、mesh exporter、BREP snapshot 编解码、STEP/IGES/STL 辅助导出等。但以下内容不应长期放在 `geometry/`：
...
- 新增 `include/cad_core/app`、`part`、`part_design`、`sketcher`、`assembly`、`base` 目录。
- 新增对应 `src/app`、`src/part`、`src/part_design`、`src/sketcher`、`src/assembly`、`src/base`。
