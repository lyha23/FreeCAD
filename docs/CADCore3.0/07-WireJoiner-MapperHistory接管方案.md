...
移除 `openExportProducerEdge` / `historyProducerChildWireCandidate` 这类 history materialization bridge，让 `MapperHistory(aHistory) -> ElementMap` 正式接管 WireJoiner 的 open-export、noOriginal、split、deleted 关系。
...
- `cad-core/include/cad_core/part/wire_joiner.h::WireJoinerHistoryMaterializationEdgeEntry` 仍有 `historyProducerChildWireCandidate` 与 `openExportProducerEdge`。
...
- 删除或替换 `WireJoinerHistoryMaterializationEdgeEntry::historyProducerChildWireCandidate`。
...
- `wire_joiner.generated_open_export_bridge.status` 从 `covered_main_path` 改为 `covered_full` 的前提：
  - `openExportProducerEdge` / `historyProducerChildWireCandidate` 已从 public capability、tests 和源码结构删除；
...
| 验收项 | 必须证明 |
| --- | --- |
| bridge 删除 | 源码和 tests 不再引用 `openExportProducerEdge` / `historyProducerChildWireCandidate` |
