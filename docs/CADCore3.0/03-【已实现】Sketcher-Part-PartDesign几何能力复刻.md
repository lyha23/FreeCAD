...
- PARTSURF 已发布 `Part::ProjectOnSurface` 当前 expected-backed slice：source-backed executor 读取 `SupportFace=App::PropertyLinkSub` 与 ordered `Projection=App::PropertyLinkSubList`，覆盖 `Mode=Edges/Faces/All`、face rebuild / hole wires、`Mode=All` Height solid、Offset placement、多 Projection ordered LinkSubList、普通 indexed `NamedShape` 和稳定 diagnostics。FreeCAD 依据是 `FeatureProjectOnSurface.cpp::tryExecute()` 的 `getSupportFace()` -> `getProjectionShapes()` -> `createProjectedWire()` -> `filterShapes()` -> `createCompound()`，以及 `projectFace()` / `createSolidIfHeight()` / `getOffsetPlacement()`；cad-core 落点为 `cad-core/src/part/part_project_on_surface.cpp`、`cad-core/src/runtime/feature_registry.cpp`、`cad-core/tools/collect_freecad_expected.py`、`cad-core/tests/test_p8_features.py`、`cad-core/tests/test_expected_fixtures.py` 和 `cad-core/src/adapters/c_api/c_api.cpp` capability metadata。验收 fixture 为 12 个 `part-project-on-surface-*` c4m1 cases 及对应 expected / focused tests；发布口径不包含 projected edge provenance mapper/history、GUI projection task panel、未验证高级分支、完整 `ProjectOnSurface` 或完整 Part surface family。
- PARTSURF 已发布 `Part::Loft` 与 `Part::Sweep` / PipeShell expected-backed 第一批能力：Loft 覆盖 `Sections`、`Solid`、`Ruled`、`Closed`、`MaxDegree`、`BRepOffsetAPI_ThruSections` 和 `loft_thru_sections` maker history；Sweep 覆盖 `Sections`、`Spine`、`Solid`、`Frenet`、`Transition`、one profile、Spine SubList compound 和 PipeShell maker history。验收 fixture 为五个 `part-loft-*`、八个 `part-sweep-*` c3m4 cases 及对应 expected / focused tests。发布口径不包含 `Linearize=true`、Loft face / vertex profile expected、复杂 profile family、Sweep multi-profile / advanced PipeShell wrapper 或完整 Part surface family。
- PARTSURF 已发布 `Part::FilledFace` helper 第一批能力：它是 cad-core 对 FreeCAD `Part.makeFilledFace()` 的 source-backed helper parity，不是原生 FreeCAD `DocumentObject`。cad-core 通过 `Boundary` / `App::PropertyLinkSubList` 读取源 wire/edge，按 `TopoShape::makeElementFilledFace()` 使用 `BRepOffsetAPI_MakeFilling`，发布 `maker_history:filling` 和 boundary source evidence；验收 fixture 为 `part-filling-closed-wire-default`、`part-filling-boundary-edges-default`、`part-filling-invalid-inputs`。发布口径不包含 `surface` / `supports` / `orders`、non-default params、non-boundary constraints、compound optional case、Surface Workbench `Surface::Filling` 或完整 Part surface family。
