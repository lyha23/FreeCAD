...
- PARTSURF 已发布 `Part::FilledFace` helper 第一批 + C5-M8-S1 能力：它是 cad-core 对 FreeCAD `Part.makeFilledFace()` 的 source-backed helper parity，不是原生 FreeCAD `DocumentObject`。cad-core 通过 `Boundary` / `App::PropertyLinkSubList` 读取源 wire/edge，按 `TopoShape::makeElementFilledFace()` 使用 `BRepOffsetAPI_MakeFilling`，发布 `maker_history:filling`、boundary source evidence、`Surface` -> `LoadInitSurface` source evidence、boundary edge `Supports` face map、G1 `Orders` map 和 locatable support/order diagnostics；验收 fixture 为 `part-filling-closed-wire-default`、`part-filling-boundary-edges-default`、`part-filling-invalid-inputs`、`c5m8/part-filling-initial-surface-boundary`、`c5m8/part-filling-support-order-edge-face`、`c5m8/part-filling-invalid-support-order`。发布口径不包含 expected-backed surface/support/order parity、G2 stable geometry、non-default params、non-boundary constraints、compound optional case、Surface Workbench `Surface::Filling` 或完整 Part surface family。
