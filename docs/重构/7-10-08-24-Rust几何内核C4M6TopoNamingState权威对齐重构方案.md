# Rust 几何内核 C4M6 TopoNamingState 权威对齐重构方案

> 基线更新时间：2026-07-10（Asia/Shanghai）
> 当前状态：实施中，不能标记为“已实现”；当前 Rust 工作树因 S3 Pad provenance 迁移中断而不能编译。
> 当前裁决：最后一个可运行 dylib 经 FreeCAD 既有 parity engine 比较为 9 case、5 green、4 red、732 diffs；更早的 Rust focused matrix 10/10 green 只代表窄 profile 通过，不是最终权威门禁。
...
当前不能把任一单独快照直接当成“当前实现结果”。必须区分三层基线：

1. **当前源码基线**：Rust HEAD `de12c53f`，工作树包含 S1–S5 未提交改动；`cargo check -p opencascade --lib` 当前失败，已确认 `response.rs` 仍引用已删除的 `part_design_pad_prism_mapped_name_provenance_like_freecad`，另有 `topo_shape_expansion.rs` 的 `NamedShape` initializer 未补新字段。旧 `mapped_name.rs` tests 也仍调用已删除函数。`NamedShape::mapped_name_provenance` 只完成了字段和空初始化，`FeatureExtrude` 尚未写入，response/topo-state projector 也尚未消费。
2. **最后可运行 dylib 基线**：`target/debug/libcad_core_ffi.dylib` SHA-256 为 `626abeb4ef64ab19e8fef3bc1d84b065a2a7665fc053efc791b055863a8484b3`。FreeCAD `RustFfiActualSource` 对该库的报告 `/private/tmp/c4m6-freecad-audit-current-lib.json` 发现 9 个 native/rejected case，5 green、4 red、732 diffs；它早于当前编译失败的 Pad ledger 半迁移，只能证明迁移前最后一个可运行状态。
3. **最后一次可复验 materialization 基线**：`cad-rs-res/manifest.json` 于 `2026-07-10T04:25:01Z` 生成，绑定 dylib SHA-256 `1ca88a23bed8e3694339835d4b04a8943df648d79038cce5ed170bdce0c319df`、build-input digest `08c008ec8497f0c40998023e9ec436721f2dfeca712e7516ac1597106f3475fe`、dirty-state digest `a25dc16b18b2122efd97aa1ca7ac3cb957fe81e50861bc66e11fb5eb5b4e3711` 和 linked OCCT `7.8`。它完整物化 10 case，Rust focused matrix 为 10/10 pass，但该 profile 没有覆盖 FreeCAD engine 后来发现的 public DTO、subshape 与 MapperHistory 差异，且它现在也不匹配当前源码/当前 dylib。

因此后续顺序不再是“先完成 S0 再从零开始 S1”，而是：先修复当前 S3 半迁移使源码恢复可编译；再完成 S3/S5 的 typed evidence 与 projector；随后重建 fresh dylib、重采 10 case，并用 FreeCAD parity engine 重新冻结权威红灯。S0 的 runner/materialization 基础设施已经存在，但 freshness 结论必须在源码重新可构建后重做。
...
| 项目 | 当前事实 | 裁决 |
| --- | --- | --- |
| Rust HEAD | `de12c53f`（`chore: 更新 C4M6 S0 新鲜运行报告`） | HEAD 之后存在大量未提交 S1–S5 改动；不能用 HEAD 或旧 receipt 代表当前源码 |
| FreeCAD HEAD | `d3e2d16789`（`refactor: 完善 FreeCAD expected 发布门禁`） | 当前方案、Rust FFI adapter/test、`cad-rs-res` 均未提交；其它大量 fixture/collector 改动属于既存用户工作，不纳入本方案编辑范围 |
| 当前 Rust 编译 | `cargo check -p opencascade --lib` 失败；已确认旧 bridge import/call 与 `NamedShape` initializer 未同步 | 当前源码不是可验证基线；必须原子完成 S3 producer/consumer/test 迁移，不能只把旧 bridge 加回来 |
...
| FreeCAD Rust-FFI report | 9 cases、5 passed、4 red、732 diffs、728 unaccepted | 当前最强语义证据；5 个 rejected/native hard-fail case green，4 个 accepted case 仍 red |

### S0–S6 状态总表

| 批次 | 当前状态 | 已落地事实 | 仍需完成/重验 |
| --- | --- | --- | --- |
| S0 | 基础设施已落地，freshness 已失效 | 10-case catalog、foreign-owner、protocol-only role、build receipt、`--materialize-dir`、artifact manifest 与 10/10 focused run 均已有磁盘证据 | 当前源码和 dylib 已越过 receipt；恢复编译后必须重新 build/materialize，并让 FreeCAD engine 消费同一 SHA |
| S1 | 实现完成度高，最后可运行库已证明核心语义 | `topo_state_admission.rs`、raw `Value` 两阶段 admission、foreign owner、encoding gate、精确三字段 rejection、C ABI status 0 已接入；FreeCAD gate 中 5 个 rejected case 全 green | 当前工作树需恢复可编译后重跑 admission/FFI 全套；OCCT response version 仍未满足“真实 runtime accessor”要求 |
| S2 | 已实现主要 publication seam，未闭合 DTO parity | `response_projection.rs`、`shape_exporter.rs`、target-only result selection、顶层 summary、mesh picking fields、默认隐藏 debug/metadata 已落地 | last gate 仍有 `results=10`、`results.subshapes=244`、`json=6`、`geometry.numeric=1`；需按 case 精确收敛，不得扩大 registry |
| S3 | **半迁移且当前编译失败** | `elementMapVersion` 已改为 `cad-core.element-map.v1`；`NamedShape` 新增非序列化 provenance map；Pad source token/operation postfix 与动态 source tag 编码雏形已加入 | `FeatureExtrude` 尚未写 ledger，response/topo-state 尚未消费；旧 bridge import/call/test 未迁移；`evidence_ledger.rs` 和 `topo_state_projection.rs` 不存在，state 仍主要从 `ObjectResult/namedShapeDebug` 反推 |
| S4 | 行为局部收敛，目标 module 未形成 | current `ElementMap` 已记录/校验 Compound owner/key/pathPrefix/childObject/target，`ResolvedNoChange` 已抑制 update/diagnostic | last gate 从 734 降到 732 且 diagnostics category 为 0，但 Compound 仍 37 diffs、ReferenceShadow 仍 250 diffs；`topo_state_reference.rs` 不存在，解析仍散落在 `topo_state.rs`/`geo_feature.rs` |
| S5 | typed projector 雏形存在，权威差异仍最大 | typed `MapperHistoryEvent`、terminal/diagnostic projection 逻辑已加入，probe diagnostic 在 last gate 已无 diagnostics diff | `MapperHistoryProjection` 仍位于 `response.rs`，旧/新投影未彻底去重；last gate 仍有 470 个 `topoNamingState.mapperHistory` diffs |
| S6 | adapter 与窄 runner 已完成，release 未完成 | FreeCAD `RustFfiActualSource`、CLI `--actual-source rust-ffi --ffi-lib` 与 4 个 adapter unit tests已落地；`python3 -m unittest tests.test_rust_ffi_expected_source` 当前 4/4 pass | 4 accepted native case 全部仍 red；当前源码无法构建；materialized artifact 已 stale；native collector/strict ledger/C++ tests/Rust live release gate 尚未作为同一最终基线全部通过 |
...
`accepted=4` 是 comparator 已批准的精确 protocol divergence 数，不代表 4 个 accepted native case 通过；报告中 4 个 accepted native case 当前全部 red。`registry_contract_failed=1` 和 728 unaccepted diffs 仍使 release gate 为 red。

### FreeCAD authority 自身当前为 green

Rust red 不能反向归因于 expected/ledger 不可用。当前 FreeCAD 侧只读复核已证明：

- collector check：`processed=9, skipped=1, failed=0`，唯一 skip 为 MapperHistory `protocol_only`；
- strict ledger validator：9 个 native/rejected pair 全部通过；
- native live release gate：9 cases、semantic green、5 个精确已注册 transport diff、0 unaccepted；
- registry contract tests：3/3 pass；
- `tests.test_freecad_expected_public_parity`：14/14 pass；
- `tests.test_topo_naming_state_response`：17/17 pass；
- ledger integrity discovery：44/44 pass；
- `tests.test_rust_ffi_expected_source`：4/4 pass。

因此当前红灯归属 Rust accepted-case projection/evidence 语义和 stale artifact，不得通过修改 expected 或扩大 ignore/registry 来消除。
...
当前实现已经具备 raw/typed 两阶段 decode、固定校验顺序、专用 rejection DTO 和 status-0 FFI 接入；最后可运行 dylib 的 5 个 rejected native case 均为 exact green。剩余工作不是重新设计 admission，而是在 S3 恢复编译后重跑单元/C ABI/live gate，并把 response `occtVersion` 从 `topo_state.rs` 的硬编码 `7.8.1` 改成真实 linked runtime accessor。
...
当前没有 `topo_naming/evidence_ledger.rs` 或 `document_graph/topo_state_projection.rs`。`NamedShape::mapped_name_provenance` 只是中断迁移留下的空 map，尚无 producer 写入和 consumer；state exporter 仍在 `response.rs` 从 `ObjectResult.named_shape_debug`、public subshape 和大量特殊 helper 反推。这一节仍是 S3 的真实目标，不得把“新增字段”记成 ledger 完成。
...
当前 Compound child-path 的 tuple evidence 与联合验证已分别落在 `ElementMap`、`topo_state.rs` 和 `geo_feature.rs`，最后 dylib 已消除 BackendGap/无变化 writeback；但 `document_graph/topo_state_reference.rs` 不存在，ReferenceShadow 完整 update 仍未对齐。因此 S4 是“行为部分通过、deep module 未收口”，不能标记完成。
...
当前 `MapperHistoryProjection` 仍定义在 `response.rs`；typed event 和 diagnostic projection 已存在，但旧 probe helper 与从 `namedShapeDebug/history_ledgers` 扩展 public history 的路径仍保留。最后可运行 dylib 的 470 个 MapperHistory diff 证明单一事实源尚未成立。
...
以下编号保留依赖关系，但不再表示每步都从零开始。当前恢复顺序是 **S3 编译修复 → S3/S5 证据投影 → S2/S4 accepted-case 收敛 → S0 fresh rebuild/materialization → S6 双门禁**；S1 只做回归复验和 OCCT runtime version 收口。

### S0：fresh binary、10-case catalog 与真实红灯

当前状态：runner/receipt/catalog/materialization 已实现并曾通过；当前 receipt、manifest 与源码/当前 dylib 不一致，freshness 为 invalid，必须在语义修改稳定后重跑。
...
5. **待重做**：当前语义实现稳定后，用 receipt-generating runner clean/fresh build，重采 10 case 并重新生成 gap matrix；不能复用 12:25 的 materialization。

S0 退出条件：12:25 的历史运行曾满足 catalog/materialization 结构条件，但当前已因源码/dylib/receipt 漂移重新变红。最终 S6 前必须重新证明完整 build receipt freshness、10 cases、正确 native/protocol role 与 foreign case；否则不得生成新的权威差异结论或发布 artifact。

### S1：`TopoStateAdmission` 与 diagnostics-only hard fail

当前状态：主要实现已落地，最后可运行 dylib 的 5 个 rejected case 全 green；保留为回归 gate，不再作为当前首要编码批次。未完成项是实际 OCCT version accessor 和当前源码恢复后的全套复验。
...
当前状态：进行中且工作树损坏。第一动作必须原子完成 Pad provenance 迁移：恢复所有 initializer、让 `FeatureExtrude` 写 ledger、让 public/topo-state projector 只读 ledger、迁移测试并删除 response bridge；完成前不得构建新 artifact 或宣称任何 fresh gate。

1. **立即修复**：完成中断的 Pad provenance 原子迁移，补齐全部 `NamedShape` initializer、producer API/call、response/topo-state consumer 与 tests，恢复 `cargo check`。
2. 把 Pad compact provenance 从 response 临时 bridge 前移到 `FeatureExtrude`/`NamedShape` producer ledger；opaque `:H...` tag 必须来自本次 producer evidence，不能由 response 固定或猜测。
...
6. 新建正式 `TopologyEvidenceLedger`/`TopoStateProjector` seam；projector 只发布 terminal entries 与明确 public history，`history_ledgers` 保持 debug-only。
...
8. **部分已落地**：`elementMapVersion=cad-core.element-map.v1` 与 producer 去除 `backend/cadRsVersion` 已完成；仍需通过 runtime accessor 发布实际 OCCT version，不能保留硬编码 `7.8.1`。
...
当前状态：tuple evidence、联合验证与 no-change 抑制已部分落地，最后 dylib 已做到 diagnostics/childElementMaps diff 为 0；仍需修 Compound elementMap status/result identity 与 ReferenceShadow 完整 update，并抽出正式 resolver seam。

1. **行为已部分落地、module 待抽出**：引入 `StateBackedReferenceResolver`，先读取已经 admission 的旧 state evidence，再用当前 request-local `NamedShape`/child map 验证 target。
...
当前状态：typed events/diagnostics projector 已有雏形，但旧 probe/debug-history 路径未删除，最后 dylib 仍有 470 个 mapperHistory diff；这是 S3 Pad ledger 之后的最高优先级。
...
3. **雏形已存在**：`MapperHistoryProjection` 生成 3 terminal entries、2 subshapes、7 events，但必须移入正式 projector seam 并由 focused contract 证明 exact equality。
...
当前状态：Rust blackbox 工具和 FreeCAD `RustFfiActualSource` adapter 已实现；release 仍为 red。FreeCAD adapter 单元测试当前 4/4 pass，native authority 自身 green，但 Rust accepted-case live gate 仍 0/4 green。
...
以下命令中的 `--materialize-dir/--artifact-manifest`、`--build-ffi/--release-gate`、`--actual-source rust-ffi` 与 `tests.test_rust_ffi_expected_source` 现在都已存在。当前问题不是 interface 缺失，而是 Rust 源码不可编译、receipt/artifact stale 和 accepted-case semantic red；任一命令不能在同一 fresh dylib 上通过都不得验收。
...
git diff --check -- \
  docs/重构/7-10-08-24-Rust几何内核C4M6TopoNamingState权威对齐重构方案.md
```
...
原始依赖顺序仍是 S0 → S1 → S2 → S3 → S4 → S5 → S6，但按当前工作树恢复时执行：

1. 原子完成 S3 Pad provenance 半迁移，恢复 `cargo check`，不得只加回旧 bridge 维持两套实现。
2. 完成 S3 typed ledger/projector 与 S5 MapperHistory 单一投影，删除 response/debug 反推和死 helper。
3. 以 FreeCAD engine 的 accepted-case keyed diff 收敛 S2 public subshape DTO 与 S4 ReferenceShadow/Compound 剩余差异。
...
达到以上条件并完成分层验收后，再保留原时间前缀把本文件重命名为：

```text
7-10-08-24-【已实现】Rust几何内核C4M6TopoNamingState权威对齐重构方案.md
```
