...
> 基线更新时间：2026-07-11（Asia/Shanghai）
> 当前状态：实施中，不能标记为“已实现”；Pad provenance 迁移已推进，但当前 Rust 工作树被 `ElementMap` 的 `StringIDRef`/snapshot 迁移阻断，尚未恢复编译。
> 当前裁决：旧 732-diff dylib 和 10/10 focused artifact 均已过期；当前 FreeCAD C4M6 strict ledger 也有 1 个 fixture、57 个错误，必须先修复持久化与 authority 闭包，再生成新的 Rust parity 基线。
...
1. **当前源码基线**：Rust HEAD `de12c53f`，工作树包含 S1–S6 未提交改动；Pad producer ledger、`evidence_ledger.rs`、`topo_state_projection.rs`、`topo_state_reference.rs` 已出现并接入部分调用链。当前 `cargo check -p opencascade --lib` 失败于 `element_map.rs`：`StringIDRef` 与旧 `String` token 混用、snapshot 缺 `mapping_sids` 序列化/恢复，共 5 个编译错误。
...
因此后续顺序调整为：先完成 `StringIDRef` 与 `ElementMapSnapshot` 持久化闭环使源码恢复可编译；再完成 S3/S5 的 typed evidence 与 projector；修复当前 FreeCAD native ledger 闭包；随后重建 fresh dylib、重采 10 case，并用 FreeCAD parity engine 重新冻结权威红灯。S0 的 runner/materialization 基础设施已经存在，但 freshness 结论必须在源码重新可构建且 native ledger 通过后重做。

### 当前硬阻断：`StringIDRef` 与 `ElementMapSnapshot` 持久化

当前 `cargo check -p opencascade --lib` 的 5 个错误集中在 `crates/opencascade/src/topo_naming/element_map.rs`，不是可忽略的测试或报告问题：
...
### FreeCAD authority 当前状态：ledger 闭包失败

当前 collector discovery 本身没有失败，但 strict ledger validator 已发现 Body Tip expected/ledger 不一致：
...
因此当前 Rust parity 尚无有效 authority verdict。先修复 FreeCAD native ledger 闭包，再以同一 expected/ledger 基线重建 Rust artifact；不得把旧 732-diff 报告或旧 10/10 focused report 继续作为当前结论。
