...
- Transformed / pattern complex ownership expected-backed：C7-M5 已发布 expected-backed closed / no backendGap；support-backed Pattern AddSubShape slot ownership 与 MultiTransform composition 在 S3 native expected 和 S4 focused / full P7 tests 中匹配 current `cad-core`。S5 未打开 C++ implementation gate，不扩大 transformed family 支持范围，不把 standalone geometry-equivalent Whole shape 升格为 native golden，也不重开旧 P7T rows。
