...
- `Profile.StableSubList=InternalFaceN` without `ReferenceShadow` 继续保持 unsupported diagnostic；没有 FreeCAD evidence 和 ElementMap-backed stable selector support 时，不把 request-local `InternalFaceN` 升级为持久 stable selector。
