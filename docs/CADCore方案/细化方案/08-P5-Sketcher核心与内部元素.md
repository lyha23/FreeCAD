...
- 当前 `FaceMakerBuildFace` 只覆盖 bounded split 主几何子集、self-intersecting edge pre-split terminal history 和 `InternalFaceN` outer-boundary generated history；`WireJoiner` 只覆盖 `noOriginal` 原始 source edge 过滤、open edge 按 face boundary split 的 fragment 子集、closed-source result-fragment ownership、terminal split / deleted history 和 open-wire carry-through；完整 `FaceMaker::postBuild()` / splitter MapperHistory、`WireJoinerP::EdgeInfo/WireInfo` ownership、open-wire history 和 source/split 账本尚未迁移。
