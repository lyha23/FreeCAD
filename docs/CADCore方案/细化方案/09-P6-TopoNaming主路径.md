...
- Sketch `InternalShape` 已对 `InternalFaceN` 记录 generated-from-outer-boundary-`EdgeN` history，hole / inner wire 不混入同一个 face 来源，且不生成 raw `FaceN` alias；raw sketch edge -> 多个 `InternalEdgeN` 的 one-source-to-many 关系记录 terminal split history，自交单边 pre-split 也记录为 raw `EdgeN` 到多个 `InternalEdgeN` 的 terminal split history，raw sketch `EdgeN/VertexN` 被过滤且没有 Internal* target 的 one-source-to-zero 关系记录 terminal deleted history；FaceMakerBuildFace 的 concrete edge / bounded-face evidence 已进入 `Sketch.InternalShape` 的 producer context 并由 topo 层优先消费为 mapper events / ElementMap diagnostics，`facemaker_history:summary_only` 只在缺 concrete evidence 时作为 diagnostic；WireJoiner child-wire event ledger、openWireCompound ownership 和 current-member vertex evidence 已进入 topo 消费路径，唯一 child-wire alias 可写 ElementMap，不可证明的一对多仍保留 `wire_joiner_current_member_vertex_multiplicity_blocked` 诊断。这些非一对一关系不写入可解析 `ElementMap`，只为 stable reference diagnostics 和 history 传播保留证据。
