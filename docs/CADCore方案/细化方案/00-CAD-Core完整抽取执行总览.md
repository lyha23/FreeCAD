...
当前 P8 只固定可用于显示、拾取和单次 recompute 的基础能力。Part primitive、ImportBrep / ImportStep / Mesh::Import STL 和 Section native oracle 已按 FreeCAD `optimalBoundingBox()` / OCCT `AddOptimal` 口径冻结，导入 shape 或 section edge 的 runtime tolerance 差异用显式 `bbox_delta` 标明；导入 shape 仍使用 indexed `NamedShape`。Part Boolean 已消费 boolean / section / generalFuse maker-history 子集，XOR / BooleanFragments expected 走 FreeCADCmd BOPTools native proxy oracle。Part Workbench surface 当前发布 source-backed `Part::Loft` first batch、source-backed `Part::Sweep` / PipeShell first batch，以及 source-backed `Part::FilledFace` helper first batch：`Part::FilledFace` 是 cad-core 对 FreeCAD `Part.makeFilledFace()` 的 helper 请求类型，不是原生 FreeCAD DocumentObject；它只发布 `Boundary` / `App::PropertyLinkSubList`、closed wire default、connected boundary edges default、`BRepOffsetAPI_MakeFilling`、`maker_history:filling`、source-backed helper metadata 和 invalid diagnostics。`Part::Sweep.Linearize=true`、advanced PipeShell wrapper、auxiliary spine、located profile、support / trihedron / binormal、Filling 的 `surface` / `supports` / `orders` / constraints / non-default params、compound optional case、Hole `ModelThread` 内部 PipeShell 和 full Part surface family 仍不是已支持目标。
...
4. 扩展 P8：完整 Joint placement / constraint、Worker / WASM / Web adapter、导入 shape 完整 ElementMap、`ShowElement=true` LinkElement / LinkGroup 持久写回事务生命周期、完整 cross-document 文档哈希 / postfix 生命周期、更复杂多层 LinkSub 链，以及 Part surface 的 `Linearize` / advanced PipeShell wrapper / Filling support-order-constraint / full surface family。
