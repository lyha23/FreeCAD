...
- S4 后续最小语义批次已关闭：执行基线为 `pwd=/Users/li/Chili3DProject/FreeCAD`，`HEAD=1ff42cb52a`（`1ff42cb52a 文档：关闭 C12-M11 S3 contract gap 分流裁决`），起点 dirty boundary 为 `<clean>`。`C12M11-BLOCKER-401` 已关闭，`C12M11-VAL-401` 已记录；历史后续最小完整语义批次定义为 `my-chili3d-C12M11-SketchEdgeTokenConsumerSync批次`、`C12-M11-StableGeometryIdMappedNameLedger设计批次` 和 `C12-M11-OpenWireRawEdgeMeshProductContract裁决批次`。S5 输入是发布 current-supported closed backend contract 与三条 follow-up 分流，不重开 closed profile backend C++ implementation，不修改 C12-M10；2026-07-05 live audit 已把 open-wire 分流项改为 current-supported。
- S5 发布闸门已关闭：执行基线为 `pwd=/Users/li/Chili3DProject/FreeCAD`，`HEAD=859608e70b`（`859608e70b 文档：关闭 C12-M11 S4 最小语义批次`），起点 dirty boundary 为 `<clean>`。Final decision：closed internal profile backend contract 发布为 `contract_current_supported/current_supported`；S2 三条 focused tests `C12M11-VAL-201..203` 已通过；edgeSegments/subshapes alignment 为 `mismatch_absent`；request-local `stableSubname` 已 passed。`C12M11-BLOCKER-501` 已关闭，`C12M11-VAL-501` 已记录；C12-M11 不授权 closed profile backend C++ implementation，不修改 C12-M10 pending 队列。
- 2026-07-05 live audit 修正：C12-M11 S2/S3 当时记录的 open wire raw `EdgeN` mesh/null 已不是当前状态。当前 `cad-core/src/sketcher/sketch_internal_result.cpp` 在 `InternalShape` 为空但 raw sketch 有边时发布 raw `EdgeN/VertexN` mesh；`tests.test_adapters.CadCoreAdapterTest.test_c_api_returns_open_sketch_raw_edge_segments`、`tests.test_adapters.CadCoreAdapterTest.test_c_api_open_wire_reference_shadow_source_identity_contract`、`tests.test_p5_sketch.CadCoreP5SketchTest.test_p5_open_sketch_keeps_raw_shape_without_profile_face` 和 open-wire geometry-id focused tests 已证明 open sketch raw `EdgeN` `subshapes[]` / `mesh.edgeSegments[]` / `stableSubname` or `index_fallback` 同源输出。`C12-M11-OpenWireRawEdgeMeshProductContract裁决批次` 不再是当前默认 follow-up。

## 最终发布状态

- C12-M11 最终状态为 `contract_current_supported/current_supported`：closed `p5/sketch-internal-face` backend response 已发布 `InternalEdgeN` edgeSegments/subshapes 与 request-local `stableSubname=EdgeN`，alignment 证据为 `mismatch_absent`。
- C12-M11 当时发布三条 follow-up：`my-chili3d-C12M11-SketchEdgeTokenConsumerSync批次`、`C12-M11-StableGeometryIdMappedNameLedger设计批次`、`C12-M11-OpenWireRawEdgeMeshProductContract裁决批次`。当前修正后，stable-id / split fragment 后端语义已由 C12-M15/C12-M16 收口，open wire raw `EdgeN` mesh 已 live current-supported；不要再把 open-wire 裁决包作为下一步默认建议。
- 后续前端同步必须消费后端 token，禁止 prefix guessing；若未来重开 open wire 后端，只能基于新的 checked-in expected/current mismatch、adapter response drift 或 `EdgeN` token/identity regression。
- C12-M10 pending 队列结论保持不变，C12-M11 不继承为 C12-M10 的自然下一包。
