...
- C12-M11 只发布三条 follow-up，不自动创建后续包：`my-chili3d-C12M11-SketchEdgeTokenConsumerSync批次`、`C12-M11-StableGeometryIdMappedNameLedger设计批次`、`C12-M11-OpenWireRawEdgeMeshProductContract裁决批次`。
- 后续前端同步必须消费后端 token，禁止 prefix guessing；stable-id 升级必须先设计 FreeCAD-style geometry id / mapped-name ledger；open wire raw `EdgeN` mesh `edgeSegments` 契约独立于 closed `InternalEdgeN`。
