...
- 当前 live 队列应从 S3 `工作步骤细分/6-30-17-02-C12-M7-S3-expected-test-capability-docs迁移实现.md` 开始；S3 必须迁移两个 C51M1 Groove UpTo product diagnostic expected、`test_p7_features.py` expected-backed assertion、`capability_contract.cpp` wording、`test_adapters.py` capability assertion 和 C12-M7 docs / 矩阵；S4-S5 仍保持 pending。
