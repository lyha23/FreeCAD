...
- S3 expected / test / capability / docs 迁移实现已完成：两个 C51M1 Groove UpTo expected 均记录 `freecad_native_parity=false` 的 product diagnostic contract，focused test 读取 expected 并保留 exact diagnostic/status 断言，capability 与 adapter assertion 发布 `product_diagnostic_contract_non_parity` 且保留 native failure note。
- S4 focused validation 与发布边界复核已完成：focused Groove test、C API capability smoke 和 capability JSON 复核均通过；`part_design.revolution_groove` 仍是 `supported_c12m7_groove_upto_product_diagnostic_contract`，narrowed gap route 为 `product_diagnostic_contract_non_parity`，并保留 fixture pair、native failure note、`freecad_native_parity=false`、delete/reopen condition。CopyOnChange / RuledSurface / full Groove family / geometry C++ parity non-goals 未误关闭。
- S5 发布闸门与后续分流已完成：最终出口为 `product_diagnostic_contract_published`，队列关闭。C12-M7 不创建 geometry implementation package；只有同一 FreeCAD / LibPack / OCCT oracle baseline 证明 Groove UpToFirst 与 UpToFace native success，且 current CAD Core 与 stable expected mismatch 时，才另开 geometry implementation candidate。
