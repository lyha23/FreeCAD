...
| C13-M5 FreeCADExpected 发布对齐 | active / S2 已完成，S3 pending | [C13-M5-FreeCADExpected发布对齐批次](C13-M5-FreeCADExpected发布对齐批次/README.md) |
...
- C13-M5 S2 已关闭：`c4m6` strict red report 保持 9 cases、2 green / 7 red，每个 red diff 都有 owner/owner_step/decision/freecad_authority/next_action/close_condition；S3 继续处理 runtime publication、mapperHistory、hash mismatch 和 stableSubname diagnostic 策略。
