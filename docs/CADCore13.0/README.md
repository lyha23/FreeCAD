...
| C13-M5 FreeCADExpected 发布对齐 | active / S4 已完成，S5 pending | [C13-M5-FreeCADExpected发布对齐批次](C13-M5-FreeCADExpected发布对齐批次/README.md) |
...
- C13-M5 S4 已关闭：comparator 已扩展到 phase family classification；首批 representative tranche 为 `c3m1`、`c10m1`、`c12m12`、`c3m5`、`c3m6`，均已按 expected discovery 重生成同名 `cad-core-res` 并生成 classified red strict report。S4 只登记 known-gap surface，不把代表 phase 宣称为 green。
- C13-M5 已出方案：`C13-M5-FreeCADExpected发布对齐批次/` 建立 release output 对齐流程，目标是让 cad-core 当前输出按 phase 对齐 `fixtures/<phase>/expected/*.freecad.json`；后续 S5 从 S4 的 family known-gap matrix 收口 release gate。
