...
SCHEMA_VERSION = "cad-core.c9m5-subshapebinder-copyonchange-native-probe.v1"
ENV_ARG_MARKER = "__cad_core_c9m5_subshapebinder_copyonchange_probe_args_env__"
ENV_ARG_NAME = "CAD_CORE_C9M5_SUBSHAPEBINDER_COPYONCHANGE_PROBE_ARGS_JSON"
PROBE_PROP = "C9M5CopyOnChangeValue"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Probe FreeCAD native SubShapeBinder BindCopyOnChange lifecycle for C9-M5.",
    )
    parser.add_argument(
        "fixture",
        nargs="?",
        default=str(ROOT / "fixtures/c9m5/subshape-binder-copyonchange-lifecycle-probe.json"),
    )
    parser.add_argument(
        "--out",
        default=str(ROOT / "fixtures/c9m5/expected/subshape-binder-copyonchange-lifecycle-probe.freecad.json"),
    )
...
def add_source_copy_on_change_property(source: Any) -> dict[str, Any]:
    ...
    evidence["add_property"] = try_call(
        ...
        lambda: source.addProperty("App::PropertyFloat", PROBE_PROP, "C9M5", "C9-M5 CopyOnChange probe property"),
    )
    try:
...
def collect_probe_payload(FreeCAD: Any, args: argparse.Namespace, command: list[str]) -> dict[str, Any]:
    ...
    try:
        doc = FreeCAD.newDocument("C9M5CopyOnChangeProbe")
        ...
        save_path = work_root / "c9m5-copyonchange-probe.FCStd"
        ...
        payload["oracle_results"]["C9M5-ORACLE-101"] = {
        ...
        payload["oracle_results"]["C9M5-ORACLE-102"] = {
        ...
        payload["oracle_results"]["C9M5-ORACLE-103"] = {
        ...
        payload["known_gap"] = {
            "kind": "c9m5_copy_on_change_full_temporary_document_cache_blocker",
            "route": "oracle_blocked",
            "reason": payload["oracle_results"]["C9M5-ORACLE-103"]["reason"],
            "source_fixture": display_path(fixture_path),
            "freecadcmd_evidence": {
                "freecad_version": version,
                "freecad_revision": revision,
        ...
        payload.update({
            ...
            "reason": str(exc),
            "traceback": traceback.format_exc(),
            "delete_condition": "Fix the C9-M5 native probe or FreeCAD runtime and rerun S3.",
...
def blocked_payload(args: argparse.Namespace, layer: str, reason: str, command: list[str], returncode: int, stdout: str = "", stderr: str = "") -> dict[str, Any]:
    return {
        ...
        "oracle_ids": ["C9M5-ORACLE-101", "C9M5-ORACLE-102", "C9M5-ORACLE-103"],
        "command": {
            "argv": command,
        ...
        "known_gap": {
            "kind": "c9m5_native_oracle_execution_blocked",
            "route": "native_oracle_blocked",
            "reason": reason,
            "freecadcmd_evidence": {
                "command": command,
