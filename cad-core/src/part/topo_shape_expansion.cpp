...
{
    // FreeCAD:
    // /Users/li/Chili3DProject/FreeCAD/src/Mod/Part/App/FeaturePartImportBrep.cpp
    // ::ImportBrep::execute() and FeaturePartImportStep.cpp::ImportStep::execute() call
    // "TopoShape aShape; aShape.import*(...)" then "this->Shape.setValue(aShape)". The import
    // path has no mapSubElement()/MapperHistory producer, so owner-qualified FaceN/EdgeN/VertexN
    // aliases would fabricate durable identity from current indexing. Keep only the indexed
    // NamedShape baseline; runtime can promote an element only when genuine map evidence arrives.
    return indexedNamedShapeForObject(owner, shape);
}
