...
using part_feature_detail::applyGlobalPlacement;
...
void executePartSphere(const app::DocumentObject& object, runtime::ComputeContext& context)
{
    ...
    if (
        !runtime::rejectUnsupportedProperties(object, context, {"Radius", "Angle1", "Angle2", "Angle3"})
    ) {
        context.objects[object.name] = {{"status", "error"}};
        return;
    }
...
void executePartCone(const app::DocumentObject& object, runtime::ComputeContext& context)
{
    ...
    if (
        !runtime::rejectUnsupportedProperties(object, context, {"Radius1", "Radius2", "Height", "Angle"})
    ) {
        context.objects[object.name] = {{"status", "error"}};
        return;
    }
