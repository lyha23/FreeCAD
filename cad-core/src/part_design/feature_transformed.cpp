...
std::vector<double> normalizedSpacingList(const app::DocumentObject& object,
                                          const std::string& property,
                                          double offset,
                                          int occurrences)
{
...
std::optional<TopoDS_Face> resolvePlanarFaceLink(const app::Link& link,
...
std::optional<TopoDS_Shape> resolveDirectionSubshape(const app::Link& link,
                                                     const app::DocumentObject& object,
                                                     runtime::ComputeContext& context,
                                                     const std::string& property)
{
...
std::optional<MirrorPlane> resolveMirrorPlane(const app::DocumentObject& object,
                                              runtime::ComputeContext& context)
{
...
std::optional<std::vector<gp_Trsf>> mirroredTransforms(const app::DocumentObject& object,
                                                       runtime::ComputeContext& context)
{
...
std::optional<TransformSource> solidSource(const std::string& objectName, runtime::ComputeContext& context)
{
    ...
    return TransformSource{objectName,
                           ...
                           namedShapeIt != context.namedShapes.end() ? std::optional<part::NamedShape>{namedShapeIt->second}
                                                                     : std::nullopt};
}

part::NamedShapeSource namedShapeSource(const TransformSource& source)
{
    return part::NamedShapeSource{source.namedShape ? source.namedShape->owner : source.owner,
                                  source.shape,
...
std::optional<part::NamedShape> namedShapeForSource(const std::string& owner,
...
std::optional<TransformSource> combineBodyPrefixSource(const app::DocumentObject& transformed,
                                                       runtime::ComputeContext& context,
                                                       ...
                                                       const std::string& owner)
{
    ...
    const auto build = part::makeElementBooleanFromSources(owner, {namedShapeSource(support), namedShapeSource(tool)}, operation);
                                                              ...
                                                              const TransformSource& source)
{
    ...
    const auto refined =
        runtime::applyRefinePropertyForOwner(*documentIt->second, feature, context, source.shape, source.namedShape);
    if (!refined) {
        return std::nullopt;
    }
...
std::optional<TransformSource> bodyPrefixSupportSource(const app::DocumentObject& transformed,
                                                       runtime::ComputeContext& context,
        ...
        if (replacesBodyShape || (shapeIt != context.shapes.end()
            ...
            if (addSubShape.addShape) {
                ...
                if (!current) {
                    ...
                    current = combineBodyPrefixSource(transformed, context, *current, tool, part::BooleanOperation::Fuse, feature);
                    if (!current) {
                        return std::nullopt;
                    }
                ...
                current = combineBodyPrefixSource(transformed, context, *current, tool, part::BooleanOperation::Cut, feature);
                if (!current) {
                    return std::nullopt;
                }
                current = refineBodyPrefixSourceIfActive(feature, context, *current);
...
std::optional<TransformSource> resolveSupportSource(const app::DocumentObject& object,
                                                    runtime::ComputeContext& context,
...
part::NamedShape transformedCopyNamedShape(const std::string& owner,
...
std::optional<TransformSource> fuseOrCutTransformedSource(const app::DocumentObject& object,
                                                          runtime::ComputeContext& context,
                                                          const TransformSource& support,
                                                          const TransformSource& tool,
                                                          part::BooleanOperation operation,
...
std::optional<TransformApplication> applyFeatureTransforms(const app::DocumentObject& object,
                                                           runtime::ComputeContext& context,
                                                           const std::vector<app::Link>& originals,
                                                           const std::vector<gp_Trsf>& copyTransforms)
{
    ...
    for (const app::Link& original : originals) {
        ...
        for (const gp_Trsf& transform : copyTransforms) {
            if (addSubIt->second.addShape) {
                ...
                auto fused =
                    fuseOrCutTransformedSource(object, context, current, *transformedTool, part::BooleanOperation::Fuse, "Originals");
                if (!fused) {
                    return std::nullopt;
                }
                current = *fused;
            ...
            if (addSubIt->second.subShape) {
                ...
                auto cut =
                    fuseOrCutTransformedSource(object, context, current, *transformedTool, part::BooleanOperation::Cut, "Originals");
                if (!cut) {
                    return std::nullopt;
                }
                current = *cut;
    ...
    part::NamedShape resultNamedShape =
        current.namedShape ? *current.namedShape : part::indexedNamedShapeForObject(current.owner, current.shape);
    if (resultNamedShape.owner != object.name) {
        resultNamedShape = part::namedShapeForPreservedSources(object.name, current.shape, {namedShapeSource(current)});
    }
    return TransformApplication{current.shape, resultNamedShape, originalNames, support->refinedFeatures};
}

std::optional<TransformApplication> applyWholeShapeTransforms(const app::DocumentObject& object,
                                                              runtime::ComputeContext& context,
                                                              ...
                                                              const std::vector<gp_Trsf>& copyTransforms)
{
    ...
    for (const gp_Trsf& transform : copyTransforms) {
        ...
        auto fused =
            fuseOrCutTransformedSource(object, context, current, *transformedTool, part::BooleanOperation::Fuse, "TransformMode");
        if (!fused) {
            return std::nullopt;
        }
        current = *fused;
...
std::optional<TransformedBuild> buildMirroredFeatures(const app::DocumentObject& object,
                                                      runtime::ComputeContext& context)
{
...
std::optional<SketchAxis> sketchAxisFromObject(const app::DocumentObject& sketch,
...
std::optional<SketchAxis> resolveSketchAxis(const app::Link& link,
                                            const app::DocumentObject& object,
                                            runtime::ComputeContext& context,
                                            const std::string& property)
{
...
std::optional<gp_Dir> resolveLinearPatternDirection(const app::DocumentObject& object,
                                                    runtime::ComputeContext& context,
                                                    const std::string& property)
{
...
std::optional<RotationAxis> resolvePolarPatternAxis(const app::DocumentObject& object,
                                                    runtime::ComputeContext& context)
{
...
std::optional<std::vector<gp_Vec>> linearPatternSteps(const app::DocumentObject& object,
                                                      runtime::ComputeContext& context,
...
std::optional<std::vector<gp_Trsf>> polarPatternTransforms(const app::DocumentObject& object,
                                                           runtime::ComputeContext& context,
                                                           int occurrences)
{
    std::vector<gp_Trsf> transforms;
...
std::optional<std::vector<gp_Trsf>> scaledTransforms(const app::DocumentObject& object,
                                                     runtime::ComputeContext& context,
                                                     const std::vector<app::Link>& originals)
{
...
std::optional<gp_Pnt> firstOriginalCenterOfMass(const app::DocumentObject& object,
                                                runtime::ComputeContext& context,
                                                const std::vector<app::Link>& originals)
{
    if (originals.empty()) {
...
std::optional<TemplateTransforms> childTemplateTransforms(const app::DocumentObject& object,
                                                          runtime::ComputeContext& context,
                                                          const std::vector<app::Link>& originals,
...
std::optional<std::vector<gp_Trsf>> multiTransformTransforms(const app::DocumentObject& object,
                                                             runtime::ComputeContext& context,
                                                             const std::vector<app::Link>& originals)
{
...
std::optional<TransformedBuild> buildLinearPatternFeatures(const app::DocumentObject& object,
                                                           runtime::ComputeContext& context)
{
...
std::optional<TransformedBuild> buildMultiTransformFeatures(const app::DocumentObject& object,
                                                            runtime::ComputeContext& context)
{
    const std::string transformMode = readEnumProperty(object, "TransformMode", {"Features", "Whole shape"}, "Features");
    const std::vector<app::Link> originals = app::readLinks(object, "Originals");
...
std::optional<TransformedBuild> buildPolarPatternFeatures(const app::DocumentObject& object,
                                                          runtime::ComputeContext& context)
{
    const std::string transformMode = readEnumProperty(object, "TransformMode", {"Features", "Whole shape"}, "Features");
...
std::optional<TransformedBuild> buildScaledFeatures(const app::DocumentObject& object,
                                                    runtime::ComputeContext& context)
{
    const std::string transformMode = readEnumProperty(object, "TransformMode", {"Features", "Whole shape"}, "Features");
    const std::vector<app::Link> originals = app::readLinks(object, "Originals");
    const auto transforms =
        scaledTransforms(object, context, transformMode == "Whole shape" ? std::vector<app::Link>{} : originals);
    if (!transforms) {
        return std::nullopt;
    }
    if (transformMode == "Whole shape") {
...
void publishTransformedResult(const app::DocumentObject& object,
                              runtime::ComputeContext& context,
...
bool applyTransformedRefine(const app::DocumentObject& object,
                            runtime::ComputeContext& context,
