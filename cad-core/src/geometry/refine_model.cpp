...
using namespace cad_core::geometry::model_refine;


void cad_core::geometry::model_refine::getFaceEdges(const TopoDS_Face& face, EdgeVectorType& edges)
{
    TopExp_Explorer it;
...
void cad_core::geometry::model_refine::boundaryEdges(const FaceVectorType& faces, EdgeVectorType& edgesOut)
{
...
TopoDS_Shell cad_core::geometry::model_refine::removeFaces(const TopoDS_Shell& shell, const FaceVectorType& faces)
{
...
namespace cad_core::geometry::model_refine
{
class WireSort
{
public:
...
TopoDS_Face FaceTypedPlane::buildFace(const FaceVectorType& faces) const
{
    ...
    std::sort(wires.begin(), wires.end(), cad_core::geometry::model_refine::WireSort());
...
FaceTypedPlane& cad_core::geometry::model_refine::getPlaneObject()
{
...
bool wireEncirclesAxis(const TopoDS_Wire& wire, const Handle(Geom_CylindricalSurface) & cylinder)
{
    ...
    // totalArc = 2 * 3.14159265358979323846 * radius: The wire encircles the axis
    return (fabs(totalArc) > 3.14159265358979323846 * radius);
}
...
TopoDS_Face FaceTypedCylinder::buildFace(const FaceVectorType& faces) const
{
    ...
    std::sort(allWires.begin(), allWires.end(), cad_core::geometry::model_refine::WireSort());
...
) const
{
    ...
    cad_core::geometry::model_refine::boundaryEdges(facesIn, normalEdges);
...
FaceTypedCylinder& cad_core::geometry::model_refine::getCylinderObject()
{
...
FaceTypedBSpline& cad_core::geometry::model_refine::getBSplineObject()
{
...
bool FaceUniter::process()
{
    ...
    cad_core::geometry::model_refine::FaceTypeSplitter splitter;
    ...
    cad_core::geometry::model_refine::FaceVectorType facesToRemove;
    cad_core::geometry::model_refine::FaceVectorType facesToSew;

    cad_core::geometry::model_refine::FaceAdjacencySplitter adjacencySplitter(workShell);

    for (typeIt = typeObjects.begin(); typeIt != typeObjects.end(); ++typeIt) {
        cad_core::geometry::model_refine::FaceVectorType typedFaces = splitter.getTypedFaceVector((*typeIt)->getType());
        cad_core::geometry::model_refine::FaceEqualitySplitter equalitySplitter;
    ...
    if (!facesToSew.empty()) {
        modifiedSignal = true;
        workShell = cad_core::geometry::model_refine::removeFaces(workShell, facesToRemove);
        TopExp_Explorer xp;
...
cad_core::geometry::BRepBuilderAPI_RefineModel::BRepBuilderAPI_RefineModel(const TopoDS_Shape& shape)
{
    myShape = shape;
    Build();
}
...
void cad_core::geometry::BRepBuilderAPI_RefineModel::Build(const Message_ProgressRange&)
#else
void cad_core::geometry::BRepBuilderAPI_RefineModel::Build()
#endif
{
    if (myShape.IsNull()) {
    ...
    if (myShape.ShapeType() == TopAbs_SOLID) {
        ...
        for (it.Init(solid, TopAbs_SHELL); it.More(); it.Next()) {
            ...
            cad_core::geometry::model_refine::FaceUniter uniter(currentShell);
            if (uniter.process()) {
        ...
        cad_core::geometry::model_refine::FaceUniter uniter(shell);
        if (uniter.process()) {
...
void cad_core::geometry::BRepBuilderAPI_RefineModel::LogModifications(const cad_core::geometry::model_refine::FaceUniter& uniter)
{
...
const TopTools_ListOfShape& cad_core::geometry::BRepBuilderAPI_RefineModel::Modified(const TopoDS_Shape& S)
{
...
Standard_Boolean cad_core::geometry::BRepBuilderAPI_RefineModel::IsDeleted(const TopoDS_Shape& S)
{
