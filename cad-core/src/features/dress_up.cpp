    ...
    for (std::size_t index = 0; index < base.link.subnames.size(); ++index) {
        ...
        if (subshape->ShapeType() != TopAbs_FACE) {
            runtime::addDiagnostic(context.diagnostics,
                                   "error",
                                   "unsupported_subshape_kind",
                                   "Draft Base requires FaceN, not " + topo::subshapeKindName(subshape->ShapeType()),
                                   object.name,
                                   "Base",
                                   "runtime",
                                   base.link.object,
        ...
        if (parsed->kind != TopAbs_FACE) {
            runtime::addDiagnostic(context.diagnostics,
                                   ...
                                   "Thickness Base currently requires FaceN, not "
                                       + topo::subshapeKindName(parsed->kind),
                                   object.name,
                                   "Base",
                                   "runtime",
                                   base.link.object,
        ...
        if (subshape->ShapeType() != TopAbs_FACE) {
            runtime::addDiagnostic(context.diagnostics,
                                   ...
                                   "Thickness Base requires FaceN, not "
                                       + topo::subshapeKindName(subshape->ShapeType()),
                                   object.name,
                                   "Base",
                                   "runtime",
                                   base.link.object,
...
std::optional<short> thicknessModeIndex(const document::DocumentObject& object,
                                        runtime::ComputeContext& context)
{
...
std::optional<short> thicknessJoinIndex(const document::DocumentObject& object,
                                        runtime::ComputeContext& context)
{
    // FreeCAD: /Users/li/Chili3DProject/重构Chili/FreeCAD/src/Mod/PartDesign/App/FeatureThickness.cpp
...
std::optional<TopoDS_Shape> resolveReferenceSubshape(const document::Link& link,
                                                     const document::DocumentObject& object,
                                                     runtime::ComputeContext& context,
                                                     const std::string& property,
                                                     TopAbs_ShapeEnum expectedKind)
{
    if (link.subnames.size() != 1U || link.subnames.front().empty()) {
        runtime::addDiagnostic(context.diagnostics,
                               "error",
                               "invalid_subshape",
                               property + " must reference exactly one " + topo::subshapeKindName(expectedKind) + " subshape",
                               object.name,
                               property,
                               "runtime",
                               link.object);
    ...
    if (parsed->kind != expectedKind) {
        runtime::addDiagnostic(context.diagnostics,
                               "error",
                               "unsupported_subshape_kind",
                               property + " requires " + topo::subshapeKindName(expectedKind) + " subshape, not "
                                   + topo::subshapeKindName(parsed->kind),
                               object.name,
                               property,
                               "runtime",
...
std::optional<gp_Dir> resolveDraftPullDirection(const document::DocumentObject& object,
                                                runtime::ComputeContext& context,
                                                const gp_Pln& neutralPlane)
{
    const auto* property = document::propertyValue(object, "PullDirection");
    ...
    const auto link = document::readLink(object, "PullDirection");
    if (!link) {
        runtime::addDiagnostic(context.diagnostics,
                               "error",
                               "missing_property",
...
std::optional<DraftNeutralPlane> resolveDraftNeutralPlane(const document::DocumentObject& object,
                                                          runtime::ComputeContext& context,
                                                          const TopoDS_Face& firstSelectedFace)
{
    const auto* property = document::propertyValue(object, "NeutralPlane");
    if (property == nullptr || property->raw.is_null()) {
        return guessDraftNeutralPlaneFromFace(firstSelectedFace, object, context);
    }

    const auto link = document::readLink(object, "NeutralPlane");
    if (!link) {
        runtime::addDiagnostic(context.diagnostics,
                               "error",
                               "missing_property",
...
std::optional<DressUpResult> buildDraft(const document::DocumentObject& object, runtime::ComputeContext& context)
{
    const auto base = resolveDressUpBase(object, context);
    if (!base) {
        return std::nullopt;
    ...
    topo::NamedShapeSource baseSource{base->link.object,
                                      base->shape,
    ...
    if (selection->faces.empty()) {
        ...
        topo::NamedShape namedShape = topo::namedShapeForPreservedSources(object.name, base->shape, {baseSource});
...
std::optional<DressUpResult> buildThickness(const document::DocumentObject& object,
                                            runtime::ComputeContext& context)
{
    const auto base = resolveDressUpBase(object, context);
    if (!base) {
    ...
    try {
        ...
        for (std::size_t solidOffset = 0; solidOffset < solids.size(); ++solidOffset) {
            ...
            topo::NamedShape solidNamedShape =
                topo::namedShapeForMakerHistory(object.name + ".Solid" + std::to_string(solidIndex),
                                                solidResultShape,
                                                {baseSource},
                                                maker);
        ...
        if (solidBuilds.size() == 1U) {
            ...
            std::vector<topo::NamedShapeSource> fuseSources;
            fuseSources.reserve(solidBuilds.size());
            for (const auto& solidBuild : solidBuilds) {
                fuseSources.push_back(topo::NamedShapeSource{
                    object.name + ".Solid" + std::to_string(solidBuild.solidIndex),
                    solidBuild.shape,
                    &solidBuild.namedShape});
            }
            const auto fuseBuild =
                topo::makeElementBooleanFromSources(object.name, fuseSources, topo::BooleanOperation::Fuse);
