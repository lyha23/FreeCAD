...
{
    std::string faceMakerClass = document::readString(object, "FaceMakerClass").value_or("");
    if (faceMakerClass.empty() && document::propertyValue(object, "FaceMakerMode") != nullptr) {
...
)
{
    ...
    switch (faceMaker) {
        case PartExtrusionFaceMaker::Simple:
            return geometry::makeSeparateFacesFromClosedWires(wires);
        case PartExtrusionFaceMaker::Cheese:
        case PartExtrusionFaceMaker::Extrusion:
            return geometry::makeCheeseFaceFromClosedWires(wires);
...
{
    const auto axisLink = document::readLink(object, "DirLink");
    ...
    if (!axisLink->subnames.empty()) {
        ...
        const auto subshape = topo::subshapeByName(axisShape, axisLink->subnames.front());
...
)
{
    ...
    if (dirMode == "Custom") {
        const auto rawDirection = document::readVector3(object, "Dir").value_or(std::array<double, 3>{0.0, 0.0, 1.0});
    ...
    if (document::readBool(object, "Reversed").value_or(false)) {
...
)
{
    ...
    if (hasActiveTaperFwd || hasActiveTaperRev) {
        ...
        // makeElementPrism(). cad-core reuses geometry::makeTaperedExtrusion for the one-side
...
void executePartExtrusion(const document::DocumentObject& object, runtime::ComputeContext& context)
{
    ...
    const auto baseLink = document::readLink(object, "Base");
    ...
    if (document::readBool(object, "Symmetric").value_or(false)) {
    ...
    nlohmann::json metadata = {{"feature", "part_extrusion"},
                               ...
                               {"reversed", document::readBool(object, "Reversed").value_or(false)},
